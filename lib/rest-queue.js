...
function createRestQueue(options = {}) {
  ...
  const userGapMs = options.gapMs;
  let gapBump = 0;
  let tail = Promise.resolve();
  let generation = 0;

  function resolveBaseGap() {
    const raw =
      typeof userGapMs === "function" ? userGapMs() : userGapMs ?? 0;
    const n = Number(raw);
    return Number.isFinite(n) && n > 0 ? n : 0;
  }

  function currentGap() {
    return Math.min(10_000, resolveBaseGap() + gapBump);
  }

  function bumpGap(extraMs = 200) {
    gapBump = Math.min(10_000, gapBump + extraMs);
  }
  ...
  return { schedule, reset, bumpGap, getGapMs: currentGap };
}
