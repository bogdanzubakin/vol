...
const MODEL_BASENAME = "sfp-regime-model";
...
const SFP_REGIME_DEFAULTS = {
  ...
  /** BTCUSDT lookback for regime trend features (hours). */
  aiRegimeBtcLookbackHours: 12,
  /** Fast BTC lookback (hours) — lower weight than slow in bootstrap. */
  aiRegimeBtcFastLookbackHours: 1,
  aiSfpRegimeFundingOiEnabled: false,
  aiSfpRegimeOnnxEnabled: false,
...
const BOOTSTRAP_WEIGHTS_BULL = [
  0.28, 0.35, 0.1, 0.42, 0.22, 0.14, -0.12, 0.38, 0.3, 0.26, 0.06, 0.16, 0.14, 0.1, 0.06, 0.04,
];

const BOOTSTRAP_WEIGHTS_BEAR = [
  0.42, 0.48, 0.14, 0.58, 0.32, 0.2, -0.1, 0.52, 0.4, 0.34, 0.1, 0.28, 0.18, 0.12, 0.08, 0.05,
];
...
function normalizeSfpRegimeConfig(raw = {}) {
  ...
  return {
    ...
    aiRegimeBtcLookbackHours: clamp(
      Math.round(num(raw.aiRegimeBtcLookbackHours, SFP_REGIME_DEFAULTS.aiRegimeBtcLookbackHours)),
      1,
      72
    ),
    aiRegimeBtcFastLookbackHours: clamp(
      Math.round(
        num(raw.aiRegimeBtcFastLookbackHours, SFP_REGIME_DEFAULTS.aiRegimeBtcFastLookbackHours)
      ),
      1,
      24
    ),
    aiSfpRegimeFundingOiEnabled: Boolean(raw.aiSfpRegimeFundingOiEnabled),
    aiSfpRegimeOnnxEnabled: Boolean(raw.aiSfpRegimeOnnxEnabled),
    aiSfpRegimeFundingOiGbmEnabled: Boolean(raw.aiSfpRegimeFundingOiGbmEnabled),
    aiSfpRegimeGbmBullThreshold: clamp(
      num(raw.aiSfpRegimeGbmBullThreshold, SFP_REGIME_DEFAULTS.aiSfpRegimeGbmBullThreshold),
      0.5,
      0.95
    ),
    aiSfpRegimeGbmBearThreshold: clamp(
      num(raw.aiSfpRegimeGbmBearThreshold, SFP_REGIME_DEFAULTS.aiSfpRegimeGbmBearThreshold),
      0.5,
      0.95
    ),
  };
...
function sfpRegimeRuntimeConfig(raw = {}) {
  const c = normalizeSfpRegimeConfig(raw);
  if (!c.aiSfpRegimeFundingOiGbmEnabled) return c;
  return {
    ...c,
    aiSfpRegimeFundingOiEnabled: true,
    aiSfpRegimeOnnxEnabled: true,
    aiSfpRegimeBullThreshold: c.aiSfpRegimeGbmBullThreshold,
    aiSfpRegimeBearThreshold: c.aiSfpRegimeGbmBearThreshold,
  };
}
...
function resolveFeatureNames(stored, regimeCfg, extras = {}) {
  if (Array.isArray(extras.featureNames) && extras.featureNames.length) {
    return extras.featureNames;
  }
  const cfg = sfpRegimeRuntimeConfig(regimeCfg ?? extras);
  if (cfg.aiSfpRegimeFundingOiEnabled) return FEATURE_NAMES_WITH_FUNDING_OI;
...
function predictSfpRegime(bars, extras = {}, model = getModel()) {
  ...
  const regimeCfg = sfpRegimeRuntimeConfig(extras.regimeCfg ?? extras);
  const stored = normalizeStoredModel(model);
  const sub = subModelForSignal(stored, signalKind);
  const featNames = resolveFeatureNames(stored, regimeCfg, extras);
  const metrics = extras.metrics ? regimeMetricsFromSignal(extras.metrics) : null;
  const fundingOi =
    extras.fundingOi ??
  ...
  const useOnnx = Boolean(regimeCfg.aiSfpRegimeOnnxEnabled);
  if (useOnnx) {
...
function evaluateSfpRegimeGate(cfg, bars, extras = {}) {
  ...
  const metrics = extras.metrics
    ...
    : null;
  const { probability, features, head, backend } = predictSfpRegime(
    bars,
    {
      ...extras,
  ...
  if (probability < threshold) {
    return { pass: true, enabled: true, probability, features, head, signalKind, backend };
  }
...
function computeFeatureStats(samples, featLen = FEATURE_NAMES.length) {
...
async function buildTrainingSamples(trades, fetchBars, options = {}) {
  ...
  const featNames =
    ...
    (cfg.aiSfpRegimeFundingOiEnabled || options.aiSfpRegimeFundingOiEnabled
      ? FEATURE_NAMES_WITH_FUNDING_OI
      : FEATURE_NAMES);
  ...
  const btcFeaturesEnabled = options.btcFeaturesEnabled !== false;
  const btcBars = btcFeaturesEnabled
    ? options.btcBars ??
  ...
  for (let i = 0; i < total; i++) {
    ...
    const features = extractSfpRegimeFeatures(window, {
      ...
      btcFastLookbackHours: cfg.aiRegimeBtcFastLookbackHours,
      signalKind: trade.signalKind,
      fundingOi: getFundingOiAt?.(trade.symbol, trade.openedAt) ?? null,
    });
    samples.push({
...
function zeroBtcFeatureWeights(sub) {
  if (!sub?.weights) return sub;
  for (const name of [
    "btcMomentum12h",
    "btcTrendStrength12h",
    "btcMomentum1h",
    ...
    if (i < 0 || i >= sub.weights.length) continue;
    sub.weights[i] = 0;
    sub.means[i] = name.includes("Momentum") ? 0.5 : 0;
    sub.stds[i] = 1;
  }
  return sub;
}
...
async function trainFromTrades(trades, fetchBars, options = {}) {
  ...
  const btcFeaturesEnabled = options.btcFeaturesEnabled !== false;
  if (!btcFeaturesEnabled) {
    zeroBtcFeatureWeights(bullModel);
    zeroBtcFeatureWeights(bearModel);
  }
...
function getModelStatus(scope = "paper") {
  ...
  return {
    ...
    featureCount: featureNamesForModel(model).length,
    gbmAvailable: Boolean(
      loadGbmEnsemble(key, "bull") && loadGbmEnsemble(key, "bear")
    ),
    backend: model.backend ?? "logistic",
    hasBtcFeatures: FEATURE_NAMES.includes("btcMomentum12h"),
    btcFeaturesActive:
      FEATURE_NAMES.includes("btcMomentum12h") &&
      (Math.abs(model.bull?.weights?.[12] ?? 0) > 1e-6 ||
        Math.abs(model.bull?.weights?.[13] ?? 0) > 1e-6 ||
        Math.abs(model.bear?.weights?.[12] ?? 0) > 1e-6 ||
        Math.abs(model.bear?.weights?.[13] ?? 0) > 1e-6),
