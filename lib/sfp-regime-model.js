...
/** Bars up to (and including) entry — for training / inference at signal time. */
...
async function buildTrainingSamples(trades, fetchBars, tradeStatsMap, options = {}) {
  const samples = [];
  const list = trades ?? [];
...
async function trainFromTrades(trades, fetchBars, options = {}) {
  ...
  let samples = await buildTrainingSamples(sfpTrades, fetchBars, statsMap, {
    onProgress,
  });
  ...
  onProgress?.({
    ...
    positive: pos.length,
    negative: neg.length,
    message: `Balancing ${samples.length} samples (${pos.length} bad / ${neg.length} good)…`,
  });
  await yieldToLoop();
