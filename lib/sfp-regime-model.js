...
const BOOTSTRAP_WEIGHTS_BULL = [
  0.28, 0.35, 0.1, 0.42, 0.22, 0.14, -0.12, 0.38, 0.3, 0.26, 0.06, 0.16, 0.14, 0.1,
];

const BOOTSTRAP_WEIGHTS_BEAR = [
  0.42, 0.48, 0.14, 0.58, 0.32, 0.2, -0.1, 0.52, 0.4, 0.34, 0.1, 0.28, 0.18, 0.12,
];
...
function zeroBtcFeatureWeights(sub) {
  if (!sub?.weights || sub.weights.length < FEATURE_NAMES.length) return sub;
  const i = FEATURE_NAMES.indexOf("btcMomentum12h");
  const j = FEATURE_NAMES.indexOf("btcTrendStrength12h");
  if (i >= 0) {
    sub.weights[i] = 0;
    sub.means[i] = 0.5;
    sub.stds[i] = 1;
