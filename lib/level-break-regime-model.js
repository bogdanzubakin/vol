...
const BOOTSTRAP_WEIGHTS_BULL = [
  0.3, 0.38, 0.12, 0.44, 0.24, 0.16, -0.1, 0.4, 0.32, 0.28, 0.18, 0.14,
];

const BOOTSTRAP_WEIGHTS_BEAR = [
  0.44, 0.5, 0.16, 0.6, 0.34, 0.22, -0.08, 0.54, 0.42, 0.36, 0.2, 0.2,
];
...
function evaluateLevelBreakRegimeGate(cfg, bars, extras = {}) {
  ...
  const { probability, features, head } = predictLevelBreakRegime(
    bars,
    { ...extras, metrics: metrics ?? extras.metrics, signalKind, modelScope },
    stored
  );
