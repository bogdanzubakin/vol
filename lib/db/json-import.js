...
function importBotState(db, botType, fileName, { hasBalance = false } = {}) {
  ...
  const configVersionId = ensureConfigVersion(db, botType, config, null);
  db.prepare(
  ...
  const insPos = db.prepare(
    `INSERT OR REPLACE INTO open_positions(id, bot_type, symbol, position_json, config_version_id, opened_at)
     VALUES(?, ?, ?, ?, ?, ?)`
  );
