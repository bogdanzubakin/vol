...
function formatNonSlTradeCloseMessage(botLabel, trade) {
...
function createTelegramNotifier(config) {
  ...
  return {
    ...
    onNonSlTradeClose(botLabel, trade) {
      if (!trade || trade.exitReason === "stop_loss") return;
      if (trade.exitReason === "exit_orders_failed") return;
      queueSend(formatNonSlTradeCloseMessage(botLabel, trade));
    },
