...
function createTelegramNotifier(config) {
  ...
  return {
    ...
    onTradeClose(botLabel, trade) {
      ...
      return queueSend(formatTradeCloseMessage(botLabel, trade));
    },
    onNonSlTradeClose(botLabel, trade) {
      return this.onTradeClose(botLabel, trade);
    },
    onExitOrdersFailed(pos, detail) {
      if (!pos) return;
      return queueSend(formatExitOrdersFailedMessage(pos, detail));
    },
    sendText(text) {
      if (!text) return Promise.resolve();
      return queueSend(String(text));
    },
  };
}
