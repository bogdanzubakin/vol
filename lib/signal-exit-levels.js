...
function resolveLegacyExitLevels(signalKind, metrics, entry, cfg, opts = {}) {
  const { cfgForSignal, isBearSignal } = require("./side-config");
  cfg = cfgForSignal(cfg, signalKind, opts);
  const short = isBearSignal(signalKind, opts);
  const confirm = metrics?.confirmKind || signalKind;
  const fbTp =
    (signalKind === "false_breakout" || signalKind === "false_breakout_bear") &&
    Number(cfg.fbTakeProfitPct) > 0
      ? Math.min(cfg.fbTakeProfitPct, cfg.takeProfitPct)
      : null;
  const obiTp =
    (signalKind === "obi" || signalKind === "obi_bear") &&
    Number(cfg.obiTakeProfitPct) > 0
      ? Math.min(cfg.obiTakeProfitPct, cfg.takeProfitPct)
      : null;
  const vwapTp =
    (signalKind === "vwap" || signalKind === "vwap_bear") &&
    Number(cfg.vwapTakeProfitPct) > 0
      ? Math.min(cfg.vwapTakeProfitPct, cfg.takeProfitPct)
      : null;
  const tpPct =
    vwapTp != null
      ? vwapTp
  ...
  if (short) {
    ...
    const bearConfirm =
      ...
      signalKind === "false_breakout_bear"
        ? "sfp_bear"
        : confirm === "pullback_bear" || signalKind === "pullback_bear"
          ? "pullback_bear"
          : signalKind === "foi_bear"
            ? metrics?.confirmKind || "sfp_bear"
            : signalKind === "obi_bear"
              ? "default"
              : signalKind;
    if (useSmart && bearConfirm === "sfp_bear") {
      return { ...resolveBearishSfpExits(metrics, entry, limits, cfg), limits };
    }
    if (useSmart && bearConfirm === "pullback_bear") {
      return {
        ...resolvePullbackBearExits(metrics, entry, limits, cfg),
        limits,
      };
    }
    if (signalKind === "obi_bear") {
      const slPct = Number(cfg.obiStopLossPct) || 1.2;
      return {
        stopLoss: entry * (1 + slPct / 100),
        takeProfit: entry * (1 - tpPct / 100),
        exitMethod: "obi_bear_pct",
        limits,
      };
    }
    if (signalKind === "tape_bear") {
      const slPct = Number(cfg.tapeStopLossPct) || 1.0;
      const tp = Number(cfg.tapeTakeProfitPct) > 0
        ? Math.min(cfg.tapeTakeProfitPct, cfg.takeProfitPct)
        : tpPct;
      return {
        stopLoss: entry * (1 + slPct / 100),
        takeProfit: entry * (1 - tp / 100),
        exitMethod: "tape_bear_pct",
        limits,
      };
    }
    if (signalKind === "vwap_bear") {
      const slPct = Number(cfg.vwapStopLossPct) || 0.7;
      return {
        stopLoss: entry * (1 + slPct / 100),
        takeProfit: entry * (1 - tpPct / 100),
        exitMethod: "vwap_bear_pct",
        limits,
      };
    }
  ...
  const bullConfirm =
    ...
    signalKind === "false_breakout"
      ? "sfp"
      : confirm === "pullback" || signalKind === "pullback"
        ? "pullback"
        : signalKind === "foi"
          ? metrics?.confirmKind || "sfp"
          : signalKind === "obi"
            ? "default"
            : signalKind;
  if (useSmart && bullConfirm === "sfp") {
    return { ...resolveSfpExits(metrics, entry, limits, cfg), limits };
  }
  if (useSmart && bullConfirm === "pullback") {
  ...
  if (signalKind === "tape") {
    const slPct = Number(cfg.tapeStopLossPct) || 1.0;
    const tp =
      Number(cfg.tapeTakeProfitPct) > 0
        ? Math.min(cfg.tapeTakeProfitPct, cfg.takeProfitPct)
        : tpPct;
    return {
      stopLoss: entry * (1 - slPct / 100),
...
function resolveExitLevels(signalKind, metrics, entry, cfg, opts = {}) {
  ...
  const skipAi =
    ...
    signalKind === "false_breakout" ||
    signalKind === "false_breakout_bear";
  if (cfg?.aiExitLevelsEnabled && !skipAi) {
    ...
    if (ai?.rejected) {
      return {
        stopLoss: null,
        takeProfit: null,
        exitMethod: "ai_levels_rejected",
        rejectReason: ai.rejectReason ?? "AI SL/TP ratio too wide",
        aiSlPct: ai.aiSlPct ?? null,
        aiTpPct: ai.aiTpPct ?? null,
      };
    }
