...
function createLiveBot(options = {}) {
  ...
  async function tryOpen(symbol, signalKind, metrics) {
    ...
    if (
      (signalKind === "level_break" || signalKind === "level_break_bear") &&
      cfg.aiLevelBreakSignalEnabled
    ) {
      const bars =
        getBarsForSymbol?.(symbol) ?? getRecentBars?.(symbol, 120) ?? [];
      const signalGate = evaluateLevelBreakSignalGate(cfg, bars, {
        metrics,
        signalKind,
        modelScope: "live",
      ...
      if (!signalGate.pass) {
        pushLog(
          ...
          symbol,
          signalGate.detail
            ? `Level-break signal AI: ${signalGate.detail}`
