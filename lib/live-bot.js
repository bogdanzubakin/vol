...
const { dayKeyUtcPlus3 } = require("./live-bot-history");
...
function createLiveBot(options = {}) {
  ...
  const foiMiddayColdPauseTracker = createFoiMiddayColdPauseTracker();
  for (const t of [...(state.closedTrades ?? [])].reverse()) {
    ...
    foiMiddayColdPauseTracker.recordClosedTrade(t);
  }
  let saveTimer = null;
  let busy = false;
  /** Coalesce price updates dropped while busy. */
  let pendingPriceUpdate = null;
  const pendingSymbols = new Set();
  const exitEnsureAt = new Map();
  const positionSyncAt = new Map();
  /** Prevent double recordClose / FOI tracker double-count. */
  ...
  function clearDailyHistory(reason = "daily") {
    ...
    if (reason !== "daily") {
      pushLog(
        ...
        "—",
        reason === "full"
          ? `Full history cleared · ${state.openPositions.length} open position(s) kept`
  ...
  function ensureRuntimeConfigVersion() {
    ...
    state.configVersionId =
      ensureConfigVersionForBot(BOT_TYPE_LIVE, state.config) ?? state.configVersionId;
  ...
  function flushPersist() {
    const gen = saveGen;
    try {
      const versionId = persistBotState(BOT_TYPE_LIVE, {
        config: state.config,
  ...
  function openSlotCount() {
    return state.openPositions.length + pendingSymbols.size;
  }

  /** True if pushing this pending symbol would exceed max (counts sibling pendings). */
  function wouldExceedMaxOpenOnPush(symbol) {
    const max = state.config.maxOpenPositions;
    if (!Number.isFinite(max)) return false;
    if (hasOpen(symbol)) return true;
    // Self is in pendingSymbols; other pendings may also push.
    const othersPending = Math.max(0, pendingSymbols.size - (pendingSymbols.has(symbol) ? 1 : 0));
    return state.openPositions.length + othersPending >= max;
  }
  ...
  function tryAcquireOpenSlot(symbol) {
    ...
    const max = state.config.maxOpenPositions;
    if (Number.isFinite(max) && openSlotCount() >= max) {
      return false;
    }
  ...
  /**
   * FOI regime / size gates (causal). Returns local sizeScale — never shared state.
   */
  function evaluateFoiOpenGates(cfg, signalKind, symbol) {
    let sizeScale = 1;
    if (!isFoiSignalKind(signalKind)) {
      return { ok: true, sizeScale: 1, detail: null };
    }
    if (cfg.foiFollowthroughRegimeEnabled) {
      const ftGate = foiFollowthroughTracker.check(cfg, Date.now());
      if (!ftGate.pass) {
        return {
          ok: false,
          sizeScale: 0,
    ...
    if (cfg.foiColdDayEnabled) {
      const coldGate = foiColdDayTracker.check(cfg, Date.now());
      if (!coldGate.pass) {
        return {
          ok: false,
          sizeScale: 0,
    ...
    if (cfg.foiMiddayColdPauseEnabled) {
      const midGate = foiMiddayColdPauseTracker.check(cfg, Date.now());
      if (!midGate.pass) {
        return {
          ok: false,
          sizeScale: 0,
          detail: midGate.detail
    ...
    if (cfg.foiBtcLookalikeEnabled) {
      const look = foiBtcLookalikeAllows(cfg, {
        symbol,
        asOfMs: Date.now(),
    ...
    const utcGate = foiUtcHourAllows(cfg, Date.now());
    if (!utcGate.ok) {
      return {
        ok: false,
        sizeScale: 0,
  ...
  /** Max isolated margin at risk for drawdown % (position margin × max slots, not notional). */
  ...
  function drawdownRealizedSinceAnchor() {
  ...
  function maybeTrailDrawdownBaseline() {
    ...
    if (state.drawdownBaseline == null) return;
    const delta = drawdownRealizedSinceAnchor();
    if (delta <= 0) return;
  ...
  /** Drawdown uses closed-trade PnL (leverage-normalized) vs trailing margin baseline. */
  function drawdownEquityFromTrades() {
    const ref = state.drawdownBaseline ?? drawdownReferenceCapital();
    return ref + drawdownRealizedSinceAnchor();
  }
  ...
  async function placeMissingTakeProfit(pos, mark, meta, cfg) {
    const closeSide = closeSideForPos(pos);
    let rawTp = pos.takeProfit;
    const tpPct =
      Number(pos.aiTpPct) > 0
        ? Number(pos.aiTpPct)
        : Number(cfg.takeProfitPct) > 0
          ? Number(cfg.takeProfitPct)
          : 3;
    const wouldTrigger = isShort(pos)
      ? wouldShortTakeProfitTrigger(mark, rawTp)
      : wouldLongTakeProfitTrigger(mark, rawTp);
    if (!Number.isFinite(rawTp) || rawTp <= 0 || wouldTrigger) {
      rawTp = isShort(pos)
        ? takeProfitFallbackForShort(pos.entryPrice, tpPct, mark, meta.tickSize)
        : takeProfitFallbackForLong(pos.entryPrice, tpPct, mark, meta.tickSize);
    }
    const tp = trader.formatPrice(rawTp, meta) ?? rawTp;
  ...
  async function ensurePositionExitOrders(pos, { force = false, exPos = null } = {}) {
    ...
    if (tpOrder) {
      ...
      changed = (await placeMissingTakeProfit(pos, mark, meta, cfg)) || changed;
    }
      ...
      const tpPct =
        Number(pos.aiTpPct) > 0
          ...
          : Number(cfg.takeProfitPct) > 0
            ...
            : 3;
      const wouldTrigger = isShort(pos)
        ? wouldShortTakeProfitTrigger(mark, tp)
        : wouldLongTakeProfitTrigger(mark, tp);
      if (wouldTrigger) {
        const fallback = isShort(pos)
          ? takeProfitFallbackForShort(
              pos.entryPrice,
        ...
        if (formatted) {
          ...
          pushLog(
            "WARN",
            pos.symbol,
            `TP rejected direction; fallback +${tpPct}% @ ${tp}`
          );
        }
      }
      try {
        const tpOrderNew = await trader.placeCloseTakeProfit(
          pos.symbol,
          tp,
          closeSide
        );
  ...
  async function closeOnExchange(pos, reason, exitPriceHint) {
    if (!pos?.id) return;
    if (closingIds.has(pos.id) || recordedCloseIds.has(pos.id)) return;
  ...
  function recordClose(pos, exitPrice, reason, closeMeta = null) {
    if (!pos?.id || recordedCloseIds.has(pos.id)) return;
    ...
    foiMiddayColdPauseTracker.recordClosedTrade(trade);
  ...
  async function tryOpen(symbol, signalKind, metrics) {
    ...
    const isBear = isBearSignal(signalKind);
    ...
    let exitsEst = resolveExitLevels(signalKind, metrics, entryEst, cfg, {
      mark: entryEst,
      modelScope: MODEL_SCOPE,
    });
    exitsEst = applyFoiHotTpProtect(exitsEst, {
      signalKind,
      entry: entryEst,
      short: isBear,
      cfg,
    ...
    if (isFoiSignalKind(signalKind)) {
      const foiGate = evaluateFoiOpenGates(cfg, signalKind, symbol);
      if (!foiGate.ok) {
    ...
    let filledPos = null;
    let exchangeFilled = false;
    let releasePendingSlot = true;
    try {
      // Re-check FOI gates immediately before sizing / order (stale after awaits).
      if (isFoiSignalKind(signalKind)) {
        const foiGate = evaluateFoiOpenGates(cfg, signalKind, symbol);
        if (!foiGate.ok) {
          pushLog("SKIP", symbol, foiGate.detail || "FOI gate");
      ...
      let exits = resolveExitLevels(signalKind, metrics, entryPrice, cfg, {
        mark,
      ...
      if (hasOpen(symbol) || wouldExceedMaxOpenOnPush(symbol)) {
        pushLog(
          "SKIP",
          symbol,
          hasOpen(symbol)
            ? "position already open (race) · closing orphan fill"
            : "max open positions reached (race) · closing orphan fill"
        );
        await abortOpenWithoutExits(
          pos,
          hasOpen(symbol) ? "duplicate open race" : "max open race after fill"
        );
        filledPos = null;
        return;
      }
      ...
      let unresolvedOrphan = false;
      if (filledPos) {
        try {
          await abortOpenWithoutExits(
            filledPos,
            e.message || "open failed after fill"
          );
        } catch (closeErr) {
          unresolvedOrphan = true;
          console.error(
            `Live bot orphan close ${symbol}:`,
            closeErr.message || closeErr
          );
          pushLog(
            "ERROR",
            symbol,
            `orphan fill may remain on exchange: ${closeErr.message || closeErr}`
          );
        }
      } else if (exchangeFilled) {
        try {
          await trader.cancelAllOrders(symbol);
          await trader.closeMarketPosition(symbol, {
            sideHint: isBear ? "SHORT" : "LONG",
            markPrice: metrics?.close,
          });
          pushLog("CLOSE", symbol, "emergency flat after open error");
        } catch (closeErr) {
          unresolvedOrphan = true;
          console.error(
            `Live bot emergency flat ${symbol}:`,
            closeErr.message || closeErr
          );
          pushLog(
            "ERROR",
      ...
      if (unresolvedOrphan) {
        // Keep slot reserved + block symbol so we never re-open on top of orphan.
        releasePendingSlot = false;
        blockSymbolForForget(symbol);
        pendingSymbols.add(symbol);
        persistSoon();
      }
    } finally {
      if (releasePendingSlot) pendingSymbols.delete(symbol);
    }
  }
  ...
  function onFoiSignal(sym, metrics) {
    ...
    void tryOpen(sym, "foi", metrics);
  }

  function onFoiBearSignal(sym, metrics) {
    if (!state.config.tradeBearishFoiSignals) return;
    void tryOpen(sym, "foi_bear", metrics);
  }

  function onObiSignal(sym, metrics) {
    if (!state.config.tradeObiSignals) return;
    void tryOpen(sym, "obi", metrics);
  }

  function onObiBearSignal(sym, metrics) {
    if (!state.config.tradeBearishObiSignals) return;
    void tryOpen(sym, "obi_bear", metrics);
  }

  function onTapeSignal(sym, metrics) {
    if (!state.config.tradeTapeSignals) return;
    void tryOpen(sym, "tape", metrics);
  }

  function onTapeBearSignal(sym, metrics) {
    if (!state.config.tradeBearishTapeSignals) return;
    void tryOpen(sym, "tape_bear", metrics);
  }
  ...
  async function reconcilePosition(pos, bar, opts = {}) {
    ...
    if (Number.isFinite(ex.high) || Number.isFinite(ex.low)) {
    ...
    const favPrice = isShort(pos) ? ex.low : ex.high;
    if (Number.isFinite(favPrice)) await tryMoveStopLoss(pos, favPrice);
    if (Number.isFinite(ex.high) && !isShort(pos)) {
      await tryAddToPosition(pos, ex.high);
    }

    {
      const trailBars =
        getBarsForSymbol?.(pos.symbol) ??
        getRecentBars?.(pos.symbol, (state.config.foiVwapTrailBars || 120) + 5) ??
        [];
      ...
      const vwapTrail = processFoiVwapTrail({
        ...
        pos,
        bars: trailBars,
        favPrice: Number.isFinite(favPrice) ? favPrice : null,
      });
      if (vwapTrail.trailed) {
        try {
          await replaceExitOrders(pos);
          pushLog("MOVE_SL", pos.symbol, vwapTrail.detail || "FOI VWAP trail");
        } catch (e) {
          pos.stopLoss = prevSl;
          pushLog("ERROR", pos.symbol, `FOI VWAP trail: ${e.message}`);
        }
      }
    }
    ...
    if (bar) {
      ...
      if (early) {
        ...
        await closeOnExchange(pos, early.reason, early.exitPrice);
        return;
      }
    ...
    const ohlc = { high: ex.high, low: ex.low, close: ex.close };
    if (stopLossHit(ohlc, pos)) {
      const trailedVwap =
        Boolean(pos.foiVwapTrailArmed) &&
        pos.initialStopLoss != null &&
        Math.abs((pos.stopLoss ?? 0) - pos.initialStopLoss) > 1e-10;
      await closeOnExchange(
        pos,
  ...
  function mergePendingPriceUpdate(prev, next) {
    if (!prev) return next;
    // null filter = all symbols wins; otherwise widen to all if filters differ.
    const symbolFilter =
      prev.symbolFilter == null || next.symbolFilter == null
        ? null
        : prev.symbolFilter === next.symbolFilter
          ? prev.symbolFilter
          : null;
    return {
      getBar: next.getBar || prev.getBar,
      symbolFilter,
      opts: { ...prev.opts, ...next.opts },
    };
  }
  ...
  async function updatePrices(getBar, symbolFilter = null, opts = {}) {
    maybeRollDailyHistory();
    if (busy) {
      pendingPriceUpdate = mergePendingPriceUpdate(pendingPriceUpdate, {
        getBar,
        symbolFilter,
        opts,
      });
    ...
    if (!state.openPositions.length && !pendingPriceUpdate) return;

    busy = true;
    let changed = false;
    try {
      let job = { getBar, symbolFilter, opts };
      while (job) {
        if (state.openPositions.length) {
          const snapshot = [...state.openPositions];
          for (const pos of snapshot) {
            if (!state.openPositions.some((p) => p.id === pos.id)) continue;
            if (recordedCloseIds.has(pos.id) || closingIds.has(pos.id)) continue;
            if (job.symbolFilter && pos.symbol !== job.symbolFilter) continue;
            const bar = job.getBar(pos.symbol);
            if (!bar?.close) continue;
            await reconcilePosition(pos, bar, job.opts);
            changed = true;
          }
        }
        if (changed) {
        ...
        job = pendingPriceUpdate;
        pendingPriceUpdate = null;
      }
    } finally {
      busy = false;
      if (pendingPriceUpdate) {
        const next = pendingPriceUpdate;
        pendingPriceUpdate = null;
        void updatePrices(next.getBar, next.symbolFilter, next.opts);
      }
    }
  }
  ...
  function forgetPositionsFromBot(positions, detail) {
    ...
    if (forgottenIds.length) {
      deleteLiveTradeIds(forgottenIds);
      persistSoon();
    }
    return forgottenIds;
  }
  ...
  async function resetHistory({ full = false } = {}) {
    if (full) {
      clearBotHistory(BOT_TYPE_LIVE);
    }
    clearDailyHistory(full ? "full" : "manual");
    return getPublicStateAsync();
  }
