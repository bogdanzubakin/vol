...
function createLiveBot(options = {}) {
  ...
  async function verifyExitOrders(pos) {
    ...
    const hasSl = trader.hasCloseStopLoss(orders);
    const hasTp = trader.hasCloseTakeProfit(orders);
    const missing = [];
  ...
  async function placeMissingTakeProfit(pos, meta) {
    ...
    try {
      const tpOrder = await trader.placeCloseTakeProfit(pos.symbol, tp);
      pos.takeProfit = tp;
  ...
  async function ensurePositionExitOrders(pos, { force = false } = {}) {
    ...
    const slOrder = trader.pickCloseStopAlgo(orders);
    const tpOrder = trader.pickCloseTakeProfitAlgo(orders);
    let changed = false;
  ...
  async function replaceExitOrders(pos) {
    ...
    if (!slOk) {
      ...
      try {
        const slOrderNew = await trader.placeCloseStop(pos.symbol, sl);
        pos.stopLoss = sl;
    ...
    if (!tpOk) {
      ...
      const tpOrderNew = await trader.placeCloseTakeProfit(pos.symbol, tp);
      pos.takeProfit = tp;
  ...
  async function tryOpen(symbol, signalKind, metrics) {
    ...
    if (
      ...
      cfg.htfContraindicationEnabled &&
      resolveHtfContraindication
    ) {
      try {
        const htf = await resolveHtfContraindication(symbol, signalKind, Date.now(), cfg);
        if (htf?.enabled && !htf.pass) {
          pushLog("SKIP", symbol, `15m blocked: ${htf.detail || "contraindication"}`);
          pendingSymbols.delete(symbol);
          return;
        }
      } catch (e) {
  ...
  async function tryMoveStopLoss(pos, price) {
    ...
    const rawTarget = entryBasedStopPrice(
    ...
    if (pricesCloseEnough(targetSl, pos.stopLoss, meta.tickSize)) return false;
  ...
  async function reconcilePosition(pos, bar) {
    ...
    await ensurePositionExitOrders(pos);
  ...
  async function updatePrices(getBar, symbolFilter = null) {
  ...
  async function syncFromExchange() {
    ...
    for (const pos of state.openPositions) {
      ...
      if (exPos && Math.abs(exPos.positionAmt) > 0) {
        ...
        still.push(pos);
        await ensurePositionExitOrders(pos, { force: true });
      } else {
