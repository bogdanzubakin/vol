...
function createLiveBot(options = {}) {
  ...
  async function tryOpen(symbol, signalKind, metrics) {
    ...
    if (isFoiSignalKind(signalKind) && cfg.foiFollowthroughRegimeEnabled) {
      ...
      pendingFoiSizeScale = Math.min(pendingFoiSizeScale, ftGate.sizeScale || 1);
    }

    if (isFoiSignalKind(signalKind) && cfg.foiColdDayEnabled) {
      ...
      pendingFoiSizeScale = Math.min(pendingFoiSizeScale, coldGate.sizeScale || 1);
    }

    if (isFoiSignalKind(signalKind) && cfg.foiBtcLookalikeEnabled) {
      ...
      if (!look.ok) {
        pushLog("SKIP", symbol, look.detail || "FOI BTC lookalike");
        pendingSymbols.delete(symbol);
        return;
      }
    }
    ...
    if (isFoiSignalKind(signalKind)) {
      ...
      if (!utcGate.ok) {
        pushLog("SKIP", symbol, `FOI blocked UTC hour ${utcGate.hour}`);
        pendingSymbols.delete(symbol);
        return;
      }
    }
    ...
    try {
      ...
      const margin = positionMarginFromConfig(
        ...
        MIN_MARGIN_USDT,
        isFoiSignalKind(signalKind) ? pendingFoiSizeScale : 1
      );
      pendingFoiSizeScale = 1;
      if (!margin) {
        pushLog(
