...
function createLiveBot(options = {}) {
  ...
  async function replaceExitOrders(pos) {
    ...
    const slOrder = await trader.placeCloseStop(pos.symbol, pos.stopLoss);
    const tpOrder = await trader.placeCloseTakeProfit(pos.symbol, pos.takeProfit);
    pos.slOrderId = slOrder?.orderId ?? null;
    pos.tpOrderId = tpOrder?.orderId ?? null;
  }
  ...
  async function tryOpen(symbol, signalKind, metrics) {
    ...
    const sl = stopLossPrice(metrics.corridorHigh, cfg.stopLossBelowCorridorPct);
    const tp = takeProfitPrice(entryEst, cfg.takeProfitPct);
    if (!sl || !tp) {
    ...
    try {
      ...
      const margin = available * (cfg.positionSizePct / 100);
      if (margin < 5) {
        pushLog("SKIP", symbol, `insufficient USDT ($${available.toFixed(2)} avail)`);
        return;
      }
      ...
      pushLog(
        "OPEN",
        symbol,
        `LONG ${signalKind} · ${qty} @ ~${entryPrice.toFixed(6)} · ${prep.leverage}x isolated · margin ~$${pos.margin.toFixed(2)} · SL ${sl.toFixed(6)} · TP ${tp.toFixed(6)}`
      );
      persistSoon();
    } catch (e) {
  ...
  async function tryAddToPosition(pos, price) {
    ...
    try {
      const bal = await trader.getUsdtBalance();
      const available = bal.available ?? bal.wallet ?? 0;
      const addMargin = available * (cfg.addOnDepositPct / 100);
      if (addMargin < 5) return false;
