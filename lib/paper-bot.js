...
function normalizeConfig(rawInput) {
  ...
  return {
    ...
    maxPullbackCorridorWidthPct: clamp(
      num(raw.maxPullbackCorridorWidthPct, DEFAULT_CONFIG.maxPullbackCorridorWidthPct),
      0,
      100
    ),
...
function createPaperBot(options = {}) {
  const { onTradeClosed, onDrawdownStop, resolveHtfContraindication, resolveExtremalSpikeGate } =
    options;
  let state = loadState();
  let saveTimer = null;
  ...
  async function tryOpenWithGates(symbol, signalKind, metrics) {
    ...
    if (
      ...
      botCfg.htfContraindicationEnabled &&
      resolveHtfContraindication
    ) {
      try {
        const htf = await resolveHtfContraindication(symbol, signalKind, Date.now(), botCfg);
