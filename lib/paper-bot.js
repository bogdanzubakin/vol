...
function createPaperBot() {
  ...
  function tryAddToPosition(pos, price) {
    ...
    pushLog(
      "ADD",
      pos.symbol,
      `+${cfg.addOnDepositPct}% deposit @ ${price.toFixed(6)} · move +${movePct.toFixed(2)}% (level ${nextLevel}%) · avg ${pos.entryPrice.toFixed(6)} · adds ${pos.addCount}`
    );
    return true;
  }
  ...
  function updatePrices(getBar) {
    ...
    if (changed || stillOpen.length !== state.openPositions.length) {
