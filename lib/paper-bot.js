...
function createPaperBot(options = {}) {
  ...
  /** Applied on next tryOpen for FOI size scaling. */
  ...
  async function tryOpenWithGates(symbol, signalKind, metrics) {
    ...
    if (isFoiSignalKind(signalKind) && botCfg.foiFollowthroughRegimeEnabled) {
    ...
    if (isFoiSignalKind(signalKind) && botCfg.foiColdDayEnabled) {
    ...
    if (isFoiSignalKind(signalKind) && botCfg.foiBtcLookalikeEnabled) {
