...
function normalizeOpenPosition(pos) {
  ...
  if (pos.foiVwapTrailArmed == null) pos.foiVwapTrailArmed = false;
...
function createPaperBot(options = {}) {
  const {
    ...
    getBtcBarsForRegime,
    getFundingOiAt,
    /** Optional async (symbol, side) => book-tape combo snapshot for FOI/SFP/PB gates. */
    resolveBookTapeCombo,
  } = options;
  let state = loadState();
  ...
  function evaluateFoiOpenGates(botCfg, signalKind, symbol) {
    ...
    if (botCfg.foiMiddayColdPauseEnabled) {
      const midGate = foiMiddayColdPauseTracker.check(botCfg, Date.now());
      if (!midGate.pass) {
        return {
          ok: false,
          sizeScale: 0,
  ...
  async function tryOpenWithGates(symbol, signalKind, metrics) {
    ...
    const bookTapeGatesOn =
      botCfg.bookTapeFoiConfirm ||
      botCfg.bookTapeSfpPbFilter ||
      botCfg.bookTapeSizeScale;
    if (bookTapeGatesOn && resolveBookTapeCombo) {
      const family = isFoiSignalKind(signalKind)
        ? "foi"
        : signalKind === "sfp" ||
            signalKind === "sfp_bear" ||
            signalKind === "pullback" ||
          ...
          ? "sfp_pb"
          : "other";
      if (family === "foi" || family === "sfp_pb") {
        try {
          const combo = await resolveBookTapeCombo(symbol, positionSide);
          const gate = evaluateBookTapeEntryGate(family, combo, botCfg);
          if (!gate.pass) {
            pushLog(
              "SKIP",
              symbol,
              `Book+Tape: ${gate.reason || "combo gate"}`
            );
            return;
          }
          if (
            ...
            sizeScale *= gate.scale;
          }
        } catch (e) {
          pushLog("WARN", symbol, `Book+Tape gate: ${e.message}`);
        }
      }
    }
  ...
  function onFalseBreakoutSignal(sym, metrics) {
    if (!state.config.tradeFalseBreakoutSignals) return;
    void tryOpenWithGates(sym, "false_breakout", metrics);
  }

  function onFalseBreakoutBearSignal(sym, metrics) {
    if (!state.config.tradeBearishFalseBreakoutSignals) return;
    void tryOpenWithGates(sym, "false_breakout_bear", metrics);
  }

  function onObiSignal(sym, metrics) {
    if (!state.config.tradeObiSignals) return;
    void tryOpenWithGates(sym, "obi", metrics);
  }

  function onObiBearSignal(sym, metrics) {
    if (!state.config.tradeBearishObiSignals) return;
    void tryOpenWithGates(sym, "obi_bear", metrics);
  }

  function onTapeSignal(sym, metrics) {
    if (!state.config.tradeTapeSignals) return;
    void tryOpenWithGates(sym, "tape", metrics);
  }

  function onTapeBearSignal(sym, metrics) {
    if (!state.config.tradeBearishTapeSignals) return;
    void tryOpenWithGates(sym, "tape_bear", metrics);
  }

  function onVwapSignal(sym, metrics) {
    if (!state.config.tradeVwapSignals) return;
    void tryOpenWithGates(sym, "vwap", metrics);
  }

  function onVwapBearSignal(sym, metrics) {
    if (!state.config.tradeBearishVwapSignals) return;
    void tryOpenWithGates(sym, "vwap_bear", metrics);
  }
  ...
  function updatePrices(getBar) {
    ...
    for (const pos of snapshot) {
      ...
      if (stopLossHit(ex, pos)) {
        ...
        closePosition(pos, pos.stopLoss, trailedVwap ? "foi_vwap_trail" : "stop_loss");
        changed = true;
        continue;
      }
