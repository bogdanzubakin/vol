...
/** Max price (initial entry minus %) that still counts as a false spike. */
function falseSpikeThresholdPrice(initialEntry, belowEntryPct) {
  if (!Number.isFinite(initialEntry) || initialEntry <= 0) return null;
  const pct = num(belowEntryPct, 0);
  return initialEntry * (1 - pct / 100);
}

/** Close when price falls to threshold below initial entry after min hold time. */
...
/** Raise stop toward avg entry after add-on (LONG only). */
