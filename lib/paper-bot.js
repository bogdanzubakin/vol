...
} = require("./level-break-signal-model");
const {
...
const DEFAULT_CONFIG = {
  ...
  /** Skip level-break when local corridor width exceeds this % (0 = off). */
  maxLevelBreakCorridorWidthPct: 10,
  /** Level-break take-profit cap % (uses takeProfitPct when unset). */
  levelBreakTakeProfitPct: 0,
...
function normalizeConfig(rawInput) {
  ...
  return {
    ...
    maxLevelBreakCorridorWidthPct: clamp(
      num(
        raw.maxLevelBreakCorridorWidthPct,
        DEFAULT_CONFIG.maxLevelBreakCorridorWidthPct
      ),
      0,
      100
    ),
    levelBreakTakeProfitPct: clamp(
      num(raw.levelBreakTakeProfitPct, DEFAULT_CONFIG.levelBreakTakeProfitPct),
      0,
      200
    ),
    ...
    tradeLevelBreakSignals:
      raw.tradeLevelBreakSignals !== undefined
        ? Boolean(raw.tradeLevelBreakSignals)
        : DEFAULT_CONFIG.tradeLevelBreakSignals,
    tradeLevelBreakBearSignals:
      raw.tradeLevelBreakBearSignals !== undefined
        ? Boolean(raw.tradeLevelBreakBearSignals)
        : DEFAULT_CONFIG.tradeLevelBreakBearSignals,
    ...
    ...normalizeLevelBreakRegimeConfig({ ...LEVEL_BREAK_REGIME_DEFAULTS, ...raw }),
    ...normalizeLevelBreakSignalConfig({ ...LEVEL_BREAK_SIGNAL_DEFAULTS, ...raw }),
...
function takeProfitPctForSignal(signalKind, cfg) {
  ...
  if (
    ...
    return Math.min(cfg.levelBreakTakeProfitPct, base);
  }
  return base;
}
...
function createPaperBot(options = {}) {
  ...
  function paperDrawdownEquity() {
    return equityFromClosedTrades(
      state.drawdownBaseline ?? state.config.initialDeposit,
      state.closedTrades
    );
  }
  ...
  async function tryOpenWithGates(symbol, signalKind, metrics) {
    ...
    if (
      (signalKind === "level_break" || signalKind === "level_break_bear") &&
      levelBreakRegimeMonitor &&
      botCfg.aiLevelBreakRegimeEnabled
    ) {
      const bars =
        getBarsForSymbol?.(symbol) ?? getRecentBars?.(symbol, 120) ?? [];
      const regimeGate = levelBreakRegimeMonitor.checkSymbol(
        symbol,
        botCfg,
        bars,
        metrics,
      ...
      if (!regimeGate.pass) {
        pushLog(
          ...
          symbol,
          regimeGate.detail
            ? `Level-break regime AI: ${regimeGate.detail}`
            : "Level-break regime AI: bad market conditions"
        );
        return;
      }
    }
    ...
    if (
      (signalKind === "level_break" || signalKind === "level_break_bear") &&
      botCfg.aiLevelBreakSignalEnabled
    ) {
      const bars =
        getBarsForSymbol?.(symbol) ?? getRecentBars?.(symbol, 120) ?? [];
      const gate = evaluateLevelBreakSignalGate(botCfg, bars, {
        metrics,
        signalKind,
        tradeStats: null,
  ...
  function onLevelBreakSignal(sym, metrics) {
    if (!state.config.tradeLevelBreakSignals) return;
    void tryOpenWithGates(sym, "level_break", metrics);
  }

  function onLevelBreakBearSignal(sym, metrics) {
    if (!state.config.tradeLevelBreakBearSignals) return;
    void tryOpenWithGates(sym, "level_break_bear", metrics);
  }
  ...
  function updatePrices(getBar) {
    ...
    for (const pos of state.openPositions) {
      ...
      if (stopLossHit(tickBar, pos)) {
        closePosition(pos, pos.stopLoss, "stop_loss");
        changed = true;
        continue;
      }
      if (!pos.runnerMode && takeProfitHit(tickBar, pos)) {
  ...
  function closeSymbol(symbol, reason = "manual") {
    ...
    const idx = state.openPositions.findIndex((p) => p.symbol === sym);
    if (idx < 0) throw new Error(`No paper position for ${sym}`);
