...
/** LONG stop at initial entry ± offset% (offset below entry = positive, above = negative). */
function entryBasedStopPrice(initialEntry, offsetPct) {
  if (!Number.isFinite(initialEntry) || initialEntry <= 0) return null;
  return initialEntry * (1 - offsetPct / 100);
}
...
function createPaperBot(options = {}) {
  ...
  function tryOpen(symbol, signalKind, metrics) {
    ...
    pushLog(
      "OPEN",
      symbol,
      `LONG ${signalKind} @ ${entry.toFixed(6)} · margin $${margin.toFixed(2)} · deposit $${state.balance.toFixed(2)} · SL ${sl.toFixed(6)} · TP ${tp.toFixed(6)}`
    );
    persistSoon();
  }
  ...
  function tryAddToPosition(pos, price) {
    ...
    pushLog(
      "ADD",
      pos.symbol,
      `+${cfg.addOnDepositPct}% ($${addMargin.toFixed(2)}) @ ${price.toFixed(6)} · deposit $${state.balance.toFixed(2)} · move +${movePct.toFixed(2)}% (level ${nextLevel}%)`
    );
    return true;
  }
