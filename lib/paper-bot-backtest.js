...
const SIM_YIELD_EVERY = 100;
const SIM_PROGRESS_MIN_MS = 4000;
...
async function runSymbolBacktest(sim, symbol, bars, signalCfg, fmOpts, extras = {}) {
  const need = minHistoryBars(signalCfg);
  if (bars.length < need) {
    return { symbol, skipped: true, reason: "insufficient_bars", bars: bars.length, signals: 0 };
  }

  const { botCfg: rawBotCfg = {}, moverBars = null, onProgress } = extras;
  ...
  let prevPb = false;
  let prevLevelBreak = false;
  let prevLevelBreakBear = false;
  let sfpEdges = 0;
  let sfpBearEdges = 0;
  let pbEdges = 0;
  let levelBreakEdges = 0;
  let levelBreakBearEdges = 0;
  ...
  for (let i = need; i < bars.length; i++) {
    ...
    const shouldReport =
      i === need ||
      i === bars.length - 1 ||
      (i - need) % SIM_YIELD_EVERY === 0 ||
      Date.now() - lastProgressAt >= SIM_PROGRESS_MIN_MS;
    if (shouldReport) {
      onProgress?.(i, bars.length);
      lastProgressAt = Date.now();
      if (i > need) await yieldToLoop();
    }

    const window = barsWindowAt(bars, i, need);
    let moverWindow = window;
    if (moverBars?.length) {
      while (
        moverEndIdx + 1 < moverBars.length &&
        moverBars[moverEndIdx + 1].closeTime <= bar.closeTime
      ) {
        moverEndIdx++;
      }
      if (moverEndIdx >= 0) {
    ...
    if (signalCfg._tradeLevelBreak) {
      const lb = analyzeLevelBreakUp(window, signalCfg);
      const lbPass = Boolean(lb.passes);
      if (
        lbPass &&
        !prevLevelBreak &&
        lb.metrics &&
        extremalSpikeAllows(bar.closeTime, window, "LONG")
      ) {
        const hadOpen = sim.hasOpen(symbol);
        sim.onLevelBreakSignal(symbol, lb.metrics, bar.closeTime);
        if (!hadOpen && sim.hasOpen(symbol)) openedThisBar = true;
        levelBreakEdges++;
      }
      prevLevelBreak = lbPass;
    }

    if (signalCfg._tradeLevelBreakBear) {
      const lbBear = analyzeLevelBreakDown(window, signalCfg);
      const lbBearPass = Boolean(lbBear.passes);
      if (
        lbBearPass &&
        !prevLevelBreakBear &&
        lbBear.metrics &&
        ...
        const hadOpen = sim.hasOpen(symbol);
        sim.onLevelBreakBearSignal(symbol, lbBear.metrics, bar.closeTime);
        if (!hadOpen && sim.hasOpen(symbol)) openedThisBar = true;
        levelBreakBearEdges++;
      }
      prevLevelBreakBear = lbBearPass;
    }
  ...
  return {
    symbol,
    bars: bars.length,
    signals: sfpEdges + sfpBearEdges + pbEdges + levelBreakEdges + levelBreakBearEdges,
    sfpSignals: sfpEdges,
    sfpBearSignals: sfpBearEdges,
    pullbackSignals: pbEdges,
    levelBreakSignals: levelBreakEdges,
    levelBreakBearSignals: levelBreakBearEdges,
...
async function runPaperBotBacktest(options) {
  ...
  signalCfg._tradeLevelBreak = Boolean(botCfg.tradeLevelBreakSignals);
  signalCfg._tradeLevelBreakBear = Boolean(botCfg.tradeLevelBreakBearSignals);
  ...
  const needs1mMovers =
    signalInterval !== "1m" &&
    Boolean(fetchKlines1mForSymbol) &&
  ...
  const regimeStatsMap = buildSymbolTradeStatsMap([]);
  const sim = createPaperBotSimulator(botCfg, {
    maxEvents: 0,
    getRecentBars: (symbol, asOf, limit = 12) => {
      const bars = barCache.get(symbol);
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (needs1mMovers) moverBarCache.delete(symbol);
