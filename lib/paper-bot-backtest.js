...
function runSymbolBacktest(sim, symbol, bars, signalCfg, fmOpts, extras = {}) {
  ...
  for (let i = need; i < bars.length; i++) {
    ...
    if (signalCfg._tradeSfp) {
      ...
      if (sfpPass && !prevSfp && sfp.metrics && extremalSpikeAllows(bar.closeTime, window)) {
    ...
    if (signalCfg._tradePullback) {
      ...
      if (pbPass && !prevPb && pb && extremalSpikeAllows(bar.closeTime, window)) {
        const hadOpen = sim.hasOpen(symbol);
...
async function runPaperBotBacktest(options) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (needs1mMovers) {
      try {
        moverBars = await fetchKlines1mForSymbol(symbol, moverBarCount);
  ...
  const result = {
    ...
    snapshotsPending: closedTrades.length > 0,
  };
