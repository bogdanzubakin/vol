...
function levelBreakPivotBars(cfg) {
  return cfg.levelBreakPivotBars ?? 3;
}

function levelBreakLookbackBars(cfg) {
  return cfg.levelBreakLookbackBars ?? 80;
}
...
function minHistoryBars(cfg) {
  ...
  const lb =
    levelBreakLookbackBars(cfg) + levelBreakPivotBars(cfg) * 2 + 10;
  return Math.max(sfp, pb, fm, lb, 120);
}
...
function sweepReclaimMetrics(ohlc, cfg) {
  ...
  // Range is prior bars only — including the current bar made corridorLow the
  // window minimum so no wick could pierce below it (sweep never fired).
  const range = rangeHighLow(ohlc.slice(0, -1), rangeBars);
...
/**
 * Bearish SFP / sweep-reject: wick above range high, then close rejects within N bars.
 */
function sweepRejectMetrics(ohlc, cfg) {
  const need = minHistoryBars(cfg);
  if (!ohlc?.length || ohlc.length < need) return null;
  ...
  for (let i = scan.length - 1; i >= 0; i--) {
    const bar = scan[i];
    if (!Number.isFinite(bar.high)) continue;
  ...
  const timely =
    sweepIdx >= 0 && barsSinceSweep != null && barsSinceSweep <= reclaimBars;
  const rejectLevel =
    sweepHigh != null ? Math.min(corridorHigh, sweepHigh) : corridorHigh;
  const rejected =
    sweepIdx >= 0 &&
    Number.isFinite(last.close) &&
    last.close < rejectLevel &&
    last.close < corridorHigh;
  const passes = Boolean(rejected && timely);
...
function analyzeSweepReject(ohlc, cfg) {
  const need = minHistoryBars(cfg);
  const checks = [];
  ...
  const m = sweepRejectMetrics(ohlc, cfg);
  if (!m) {
    return { passes: false, metrics: null, checks };
  }

  const swept = m.sweepHigh != null;
  checks.push(
    {
      id: "sfpBearSweep",
      label: `Sweep above range high (≥${cfg.sfpMinSweepPct ?? 0.02}% wick)`,
      pass: swept,
      detail: swept
        ? `high ${m.sweepHigh?.toFixed(6)} > ${m.sweepThreshold?.toFixed(6)}`
        : "no sweep in lookback",
    },
    {
      id: "sfpBearReject",
      label: "Close rejects below sweep / range high",
      pass: m.rejected,
      detail: m.rejected
        ? `${m.close} < ${Math.min(m.corridorHigh, m.sweepHigh ?? m.corridorHigh).toFixed(6)}`
        : `${m.close} vs high ${m.corridorHigh}`,
    },
    {
      id: "sfpBearTimely",
      label: `Reject within ${m.reclaimBars} bars of sweep`,
      pass: m.timely,
      detail:
        m.barsSinceSweep != null
          ? `${m.barsSinceSweep} bars since sweep`
...
/** Bearish fast-mover pullback: downtrend + MA touch from above + rejection candle. */
function fastMoverPullbackBearMetrics(ohlc, cfg, fmOpts = {}, moverBars = null) {
  const fmSource = moverBars ?? ohlc;
  const fm = fastMoverMetrics(fmSource, {
    fastMoveLookbackCandles:
      fmOpts.fastMoveLookbackCandles ?? cfg.fastMoveLookbackCandles,
  ...
  const minLinear = fmOpts.minLinearChangePct ?? cfg.minLinearChangePct ?? 0;
  if (minLinear > 0 && fm.linearChangePct > -minLinear) return null;

  const maBars = cfg.pullbackMaBars ?? 7;
  const touchLookback = cfg.pullbackTouchLookback ?? 12;
  const maxDistPct = cfg.pullbackMaxDistancePct ?? 0.35;
  const maxBelowMaPct = cfg.pullbackMaxBelowMaPct ?? cfg.pullbackMaxAboveMaPct ?? 1.5;
  const need = Math.max(maBars, touchLookback) + 5;
  if (ohlc.length < need) return null;
  ...
  const touchWindow = ohlc.slice(-touchLookback);
  const touchedMa = touchWindow.some((b) => {
    if (!Number.isFinite(b.high)) return false;
    const distPct = (Math.abs(b.high - ma) / ma) * 100;
    return distPct <= maxDistPct;
  });

  const distFromMaPct = ((last.close - ma) / ma) * 100;
  const rejection = last.close < last.open;
  const nearMa = distFromMaPct <= 0 && distFromMaPct >= -maxBelowMaPct;
  const passes = Boolean(touchedMa && rejection && nearMa);
...
function isPivotHighAt(ohlc, i, pivotBars) {
  const h = ohlc[i].high;
  if (!Number.isFinite(h)) return false;
  for (let j = i - pivotBars; j <= i + pivotBars; j++) {
    if (j === i) continue;
    if (j < 0 || j >= ohlc.length) return false;
    if (ohlc[j].high > h) return false;
  }
  return true;
}

function isPivotLowAt(ohlc, i, pivotBars) {
  const l = ohlc[i].low;
  if (!Number.isFinite(l)) return false;
  for (let j = i - pivotBars; j <= i + pivotBars; j++) {
    if (j === i) continue;
    if (j < 0 || j >= ohlc.length) return false;
    if (ohlc[j].low < l) return false;
  }
  return true;
}

function countLevelTouches(ohlc, level, touchPct, fromIdx, toIdx) {
  const band = level * (touchPct / 100);
  const lo = level - band;
  const hi = level + band;
  let touches = 0;
...
function approachedLevelFromBelow(ohlc, level, approachPct, approachBars, endIdx) {
  const start = Math.max(0, endIdx - approachBars + 1);
  const floor = level * (1 - approachPct / 100);
  for (let i = start; i <= endIdx; i++) {
    const b = ohlc[i];
    if (!Number.isFinite(b.high) || !Number.isFinite(b.close)) continue;
    if (b.high >= floor && b.close < level) return true;
  }
  return false;
}

function approachedLevelFromAbove(ohlc, level, approachPct, approachBars, endIdx) {
  const start = Math.max(0, endIdx - approachBars + 1);
  const ceiling = level * (1 + approachPct / 100);
  for (let i = start; i <= endIdx; i++) {
    const b = ohlc[i];
    if (!Number.isFinite(b.low) || !Number.isFinite(b.close)) continue;
    if (b.low <= ceiling && b.close > level) return true;
  }
  return false;
}

/**
 * Bullish level break: strong pivot-high resistance tested, then close breaks above.
 */
function levelBreakUpMetrics(ohlc, cfg) {
  const need = minHistoryBars(cfg);
  if (!ohlc?.length || ohlc.length < need) return null;

  const pivotBars = levelBreakPivotBars(cfg);
  const lookback = levelBreakLookbackBars(cfg);
  const minTouches = cfg.levelBreakMinTouches ?? 2;
  const touchPct = cfg.levelBreakTouchPct ?? 0.25;
  const breakMinPct = cfg.levelBreakMinPct ?? 0.08;
  const approachPct = cfg.levelBreakApproachPct ?? 0.4;
  const approachBars = cfg.levelBreakApproachBars ?? 12;

  const last = ohlc[ohlc.length - 1];
  const prev = ohlc[ohlc.length - 2];
  if (!Number.isFinite(last?.close) || !Number.isFinite(prev?.close)) return null;

  const toIdx = ohlc.length - 2;
  const fromIdx = Math.max(pivotBars, toIdx - lookback + 1);
  const breakAt = (price) => price * (1 + breakMinPct / 100);

  let best = null;
  for (let i = fromIdx; i <= toIdx; i++) {
    if (!isPivotHighAt(ohlc, i, pivotBars)) continue;
    const level = ohlc[i].high;
    const touches = countLevelTouches(ohlc, level, touchPct, fromIdx, toIdx);
    if (touches < minTouches) continue;
    if (level >= last.close) continue;
    if (prev.close >= level) continue;
    if (last.close <= breakAt(level)) continue;
    if (!approachedLevelFromBelow(ohlc, level, approachPct, approachBars, toIdx)) {
      continue;
    }
    if (
      !best ||
      touches > best.levelTouches ||
      (touches === best.levelTouches && level > best.levelPrice)
    ) {
      best = { levelPrice: level, levelTouches: touches, pivotIdx: i };
    }
  }

  if (!best) return null;

  const range = rangeHighLow(
    ohlc.slice(0, -1),
    Math.min(lookback, ohlc.length - 1)
  );
  if (!range || range.corridorHigh <= range.corridorLow) return null;
  const localCw = corridorWidthFromWindow(
    ohlc.slice(-Math.min(120, ohlc.length))
  ...
  return {
    ...
    ...localCw,
    close: last.close,
    levelPrice: +best.levelPrice.toFixed(6),
    levelTouches: best.levelTouches,
    breakMinPct,
    levelBreak: true,
    passes: true,
    bars: ohlc.length,
...
/**
 * Bearish level break: strong pivot-low support tested, then close breaks below.
 */
function levelBreakDownMetrics(ohlc, cfg) {
  const need = minHistoryBars(cfg);
  if (!ohlc?.length || ohlc.length < need) return null;
  ...
  let best = null;
  for (let i = fromIdx; i <= toIdx; i++) {
    if (!isPivotLowAt(ohlc, i, pivotBars)) continue;
    const level = ohlc[i].low;
    const touches = countLevelTouches(ohlc, level, touchPct, fromIdx, toIdx);
    if (touches < minTouches) continue;
    if (level <= last.close) continue;
    if (prev.close <= level) continue;
    if (last.close >= breakAt(level)) continue;
    if (!approachedLevelFromAbove(ohlc, level, approachPct, approachBars, toIdx)) {
      continue;
    }
    if (
      !best ||
...
function analyzeLevelBreakUp(ohlc, cfg) {
  const need = minHistoryBars(cfg);
  const checks = [];
  ...
  const m = levelBreakUpMetrics(ohlc, cfg);
  if (!m) {
    checks.push({
      id: "levelBreak",
      label: "Resistance break (pivot high)",
      pass: false,
      detail: "no qualifying level",
    });
    return { passes: false, metrics: null, checks };
  }
  ...
  checks.push(
    {
      id: "levelTouches",
      label: `Level touches (≥${cfg.levelBreakMinTouches ?? 2})`,
      pass: true,
      detail: `${m.levelTouches} @ ${m.levelPrice}`,
    },
    {
      id: "levelBreak",
      label: `Close breaks above level (+${cfg.levelBreakMinPct ?? 0.08}%)`,
      pass: true,
      detail: `close ${m.close} > ${m.levelPrice}`,
    }
  );

  return { passes: true, metrics: m, checks };
}

function analyzeLevelBreakDown(ohlc, cfg) {
  const need = minHistoryBars(cfg);
  const checks = [];
  ...
  const m = levelBreakDownMetrics(ohlc, cfg);
  if (!m) {
    checks.push({
      id: "levelBreakBear",
      label: "Support break (pivot low)",
      pass: false,
      detail: "no qualifying level",
    });
    return { passes: false, metrics: null, checks };
  ...
  checks.push(
    ...
    {
      id: "levelBreakBear",
      label: `Close breaks below level (−${cfg.levelBreakMinPct ?? 0.08}%)`,
      pass: true,
      detail: `close ${m.close} < ${m.levelPrice}`,
    }
  );
...
const LIVE_CONFIG_KEYS = [
  ...
  "topMoveMinPct",
  "levelBreakPivotBars",
  "levelBreakLookbackBars",
  "levelBreakMinTouches",
  "levelBreakTouchPct",
  "levelBreakMinPct",
  "levelBreakApproachPct",
  "levelBreakApproachBars",
];
...
function validateLiveConfigPatch(patch) {
  ...
  const rules = {
    ...
    pullbackMaxBelowMaPct: { min: 0.1, max: 20, int: false },
    topMoveMinPct: { min: 0.1, max: 1000, int: false },
    levelBreakPivotBars: { min: 1, max: 12, int: true },
    levelBreakLookbackBars: { min: 20, max: 300, int: true },
    levelBreakMinTouches: { min: 1, max: 20, int: true },
    levelBreakTouchPct: { min: 0.05, max: 2, int: false },
    levelBreakMinPct: { min: 0.01, max: 2, int: false },
    levelBreakApproachPct: { min: 0.05, max: 3, int: false },
    levelBreakApproachBars: { min: 3, max: 60, int: true },
  };
