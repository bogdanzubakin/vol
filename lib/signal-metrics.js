...
/** Open and close both above corridor top (range high of lookback window). */
function candleFullyAboveCorridor(bar, corridorHigh) {
  const o = bar?.open;
  const c = bar?.close;
  return (
    Number.isFinite(o) &&
    Number.isFinite(c) &&
    Number.isFinite(corridorHigh) &&
    o > corridorHigh &&
    c > corridorHigh
  );
}

function countCandlesFullyAbove(bars, corridorHigh) {
  return bars.filter((b) => candleFullyAboveCorridor(b, corridorHigh)).length;
}
...
function fastCorridorMetrics(ohlc, cfg, options = {}) {
  const fm = fastMoverMetrics(ohlc, {
    ...
    minLinearChangePct: options.minLinearChangePct ?? 0,
...
function volSpikeMetrics(ohlc, cfg) {
  ...
  const aboveCorridorCount = countCandlesFullyAbove(signal, corridorHigh);
  const aboveCorridorBreakout = aboveCorridorCount >= n;
  ...
  const passesExceptBreak =
    corridorFlat &&
    aboveCorridorCount >= Math.max(1, n - 1) &&
    bullish &&
  ...
  return {
    ...
    aboveCorridorCount,
    aboveCorridorBreakout,
    minAboveCorridorCandles: n,
    ...volNearBreak,
    bullish,
    bullishCount,
    bullishLookback,
...
function analyzeVolSpike(ohlc, cfg, options = {}) {
  const n = cfg.signalCandles;
  const excludeMin = cfg.corridorExcludeMinutes ?? 40;
  const need = minHistoryBars(cfg);
  const checks = [];
  ...
  checks.push(
    ...
    {
      id: "aboveCorridorBreakout",
      label: `≥${m.minAboveCorridorCandles} candles open & close above corridor high`,
      pass: m.aboveCorridorBreakout,
      detail: `${m.aboveCorridorCount} / ${m.minAboveCorridorCandles}`,
    },
    {
      id: "bullish",
  ...
  if (cfg.regimeFilterEnabled && regime) {
    const mode = regime.mode ?? cfg.regimeMode ?? "bullish";
    const op = mode === "bearish" ? "<" : "≥";
    checks.push({
      id: "regime",
      label: `BTC regime ${mode} (close ${op} MA${regime.maBars ?? cfg.regimeMaBars ?? 20})`,
      pass: Boolean(regime.pass),
      detail: regime.detail ?? regime.label ?? "—",
    });
    if (!regime.pass) passes = false;
  }

  return { passes, metrics: m, checks };
}
...
/**
 * SFP / sweep-reclaim: wick below corridor low, then close reclaims above low within N bars.
 */
function sweepReclaimMetrics(ohlc, cfg) {
  ...
  const corridorHigh = Math.max(...corridor.map((b) => b.high));
  const corridorLow = Math.min(...corridor.map((b) => b.low));
  if (corridorHigh <= corridorLow) return null;
...
function analyzeSweepReclaim(ohlc, cfg) {
  ...
  checks.push(
    {
      id: "sfpSweep",
      label: `Sweep below corridor low (≥${cfg.sfpMinSweepPct ?? 0.02}% wick)`,
      pass: swept,
      detail: swept
    ...
    {
      id: "sfpReclaim",
      label: "Close reclaims above sweep / corridor low",
      pass: m.reclaimed,
      detail: m.reclaimed
...
/**
 * Fast-mover pullback: symbol is a fast mover, price touched short MA, bullish bounce.
 */
function fastMoverPullbackMetrics(ohlc, cfg, fmOpts = {}) {
  const fm = fastMoverMetrics(ohlc, {
  ...
  const nearMa =
    distFromMaPct >= 0 && distFromMaPct <= maxAboveMaPct;
  const passes = Boolean(touchedMa && bounce && nearMa);

  const localCw = corridorWidthFromWindow(ohlc.slice(-Math.min(120, ohlc.length)));
  if (!localCw) return null;
...
function analyzePullback(ohlc, cfg, fmOpts = {}) {
  const maBars = cfg.pullbackMaBars ?? 7;
  const touchLookback = cfg.pullbackTouchLookback ?? 12;
  ...
  const m = fastMoverPullbackMetrics(ohlc, cfg, fmOpts);
  if (!m) {
    return { passes: false, metrics: null, checks };
  }
...
function validateLiveConfigPatch(patch) {
  ...
  const rules = {
    ...
    regimeFilterEnabled: { min: 0, max: 1, int: true },
    regimeMaBars: { min: 5, max: 200, int: true },
  ...
  for (const key of LIVE_CONFIG_KEYS) {
    ...
    if (key === "regimeMode") {
      const mode = String(patch[key]).toLowerCase();
      if (mode !== "bullish" && mode !== "bearish") {
        throw new Error('regimeMode must be "bullish" or "bearish"');
      }
      out[key] = mode;
      continue;
    }
