...
function minHistoryBarsRaw(cfg) {
  ...
  const fb =
    (cfg.fbRangeBars ?? 45) + (cfg.fbLookbackBars ?? 30) + 5;
  return Math.max(sfp, pb, fm, fb, 120);
}

function minHistoryBars(cfg) {
  // Account for optional *Bull / *Bear detection overrides so windows stay large enough.
  try {
    const { cfgForSignal } = require("./side-config");
    return Math.max(
      minHistoryBarsRaw(cfg),
      minHistoryBarsRaw(cfgForSignal(cfg, "sfp")),
      minHistoryBarsRaw(cfgForSignal(cfg, "sfp_bear")),
      minHistoryBarsRaw(cfgForSignal(cfg, "pullback")),
      minHistoryBarsRaw(cfgForSignal(cfg, "pullback_bear"))
    );
  } catch {
    return minHistoryBarsRaw(cfg);
  }
}
...
function pullbackCorridorBars(cfg) {
  const n = Number(cfg?.pullbackCorridorBars ?? 120);
  return Number.isFinite(n) ? Math.max(24, Math.min(576, Math.round(n))) : 120;
}

function pullbackCorridorWindow(ohlc, cfg) {
  const n = pullbackCorridorBars(cfg);
  return corridorWidthFromWindow(ohlc.slice(-Math.min(n, ohlc.length)));
}
...
function sweepReclaimMetrics(ohlc, cfg) {
  const { cfgForSignal } = require("./side-config");
  cfg = cfgForSignal(cfg, "sfp");
  const need = minHistoryBarsRaw(cfg);
...
function analyzeSweepReclaim(ohlc, cfg) {
  const { cfgForSignal } = require("./side-config");
  const sideCfg = cfgForSignal(cfg, "sfp");
  const need = minHistoryBars(cfg);
  const checks = [];
  ...
  checks.push(
    {
      id: "sfpSweep",
      label: `Sweep below range low (≥${sideCfg.sfpMinSweepPct ?? 0.02}% wick)`,
      pass: swept,
      detail: swept
...
function analyzeSweepReject(ohlc, cfg) {
  const { cfgForSignal } = require("./side-config");
  const sideCfg = cfgForSignal(cfg, "sfp_bear");
  const need = minHistoryBars(cfg);
  const checks = [];
  ...
  checks.push(
    {
      id: "sfpBearSweep",
      label: `Sweep above range high (≥${sideCfg.sfpMinSweepPct ?? 0.02}% wick)`,
      pass: swept,
      detail: swept
    ...
    },
    {
      ...
      pass: m.rejected,
      detail: m.rejected
        ? `${m.close} < ${Math.min(m.corridorHigh, m.sweepHigh ?? Infinity).toFixed(6)}`
...
function fastMoverPullbackMetrics(ohlc, cfg, fmOpts = {}, moverBars = null) {
  ...
  const localCw = pullbackCorridorWindow(ohlc, cfg);
  ...
  return {
    ...
    maxAboveMaPct,
    bars: ohlc.length,
    pullbackCorridorBars: pullbackCorridorBars(cfg),
  };
}
...
function fastMoverPullbackBearMetrics(ohlc, cfg, fmOpts = {}, moverBars = null) {
  const { cfgForSignal } = require("./side-config");
  cfg = cfgForSignal(cfg, "pullback_bear");
  const fmSource = moverBars ?? ohlc;
  const fm = fastMoverMetrics(fmSource, {
    fastMoveLookbackCandles:
      fmOpts.fastMoveLookbackCandles ?? cfg.fastMoveLookbackCandles,
...
const { DETECTION_OVERRIDE_BASE_KEYS } = require("./side-config");

const DETECTION_SIDE_OVERRIDE_KEYS = DETECTION_OVERRIDE_BASE_KEYS.flatMap((base) => [
  `${base}Bear`,
  `${base}Bull`,
]);
...
function pickLiveConfig(cfg) {
  ...
  for (const k of DETECTION_SIDE_OVERRIDE_KEYS) {
    if (cfg[k] === undefined) continue;
    out[k] = cfg[k] === null || cfg[k] === "" ? null : cfg[k];
  }
  return out;
}
...
function validateLiveConfigPatch(patch) {
  ...
  const rules = {
    ...
    pullbackCorridorBars: { min: 24, max: 576, int: true },
  ...
  for (const key of DETECTION_SIDE_OVERRIDE_KEYS) {
    if (patch[key] === undefined) continue;
    if (patch[key] === null || patch[key] === "") {
      out[key] = null;
      continue;
    }
    const base = key.replace(/(Bear|Bull)$/, "");
    const rule = rules[base];
    if (!rule) throw new Error(`Unknown override key: ${key}`);
    let v = Number(patch[key]);
    if (!Number.isFinite(v)) throw new Error(`${key} must be a number or empty`);
    if (rule.int) v = Math.round(v);
    if (v < rule.min || v > rule.max) {
      throw new Error(`${key} must be between ${rule.min} and ${rule.max}`);
    }
