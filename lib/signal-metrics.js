...
function corridorExcludeBars(cfg) {
  const excludeMin = cfg.corridorExcludeMinutes ?? 15;
...
function volSpikeMetrics(ohlc, cfg) {
  ...
  const passesExceptBreak = corridorFlat && volSpike && bullish;
  ...
  return {
    ...
    breakGapPct,
    bars: ohlc.length,
    passes: corridorFlat && volSpike && bullish && breaksCorridor,
  };
}
