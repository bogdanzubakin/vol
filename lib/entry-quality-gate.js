...
function evaluateEntryQuality(signalKind, metrics, entry, stopLoss, takeProfit, cfg = {}) {
  ...
  if (signalKind === "level_break" || signalKind === "level_break_bear") {
    const maxCw = num(cfg.maxLevelBreakCorridorWidthPct, 0);
    const cw = metrics?.corridorWidthPct;
    if (maxCw > 0 && Number.isFinite(cw) && cw > maxCw) {
      reasons.push(`level-break corridor ${cw}% > max ${maxCw}%`);
    }
  }
