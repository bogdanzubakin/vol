...
function evaluateEntryQuality(signalKind, metrics, entry, stopLoss, takeProfit, cfg = {}) {
  ...
  if (signalKind === "foi" || signalKind === "foi_bear") {
    const confirm = metrics?.confirmKind || "";
    const useSfp = confirm === "sfp" || confirm === "sfp_bear";
    const maxCw = num(
      useSfp ? cfg.maxSfpCorridorWidthPct : cfg.maxPullbackCorridorWidthPct,
      0
    );
    const cw = metrics?.corridorWidthPct;
    if (maxCw > 0 && Number.isFinite(cw) && cw > maxCw) {
      reasons.push(`FOI corridor ${cw}% > max ${maxCw}%`);
    }
  }
