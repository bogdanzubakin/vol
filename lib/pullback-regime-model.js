...
const BOOTSTRAP_WEIGHTS_BULL = [
  0.3, 0.38, 0.12, 0.44, 0.24, 0.16, -0.1, 0.4, 0.32, 0.28, 0.18, 0.14, 0.14, 0.1, 0.06, 0.04,
  -0.2, -0.25, -0.15,
];

const BOOTSTRAP_WEIGHTS_BEAR = [
  0.44, 0.5, 0.16, 0.6, 0.34, 0.22, -0.08, 0.54, 0.42, 0.36, 0.2, 0.2, 0.18, 0.12, 0.08, 0.05,
  -0.22, -0.28, -0.18,
];
...
function normalizePullbackRegimeConfig(raw = {}) {
  ...
  return {
    ...
    aiRegimeBtcFastLookbackHours: clamp(
      Math.round(
        num(raw.aiRegimeBtcFastLookbackHours, PULLBACK_REGIME_DEFAULTS.aiRegimeBtcFastLookbackHours)
      ),
      1,
      24
    ),
...
function predictPullbackRegime(bars, extras = {}, model = getModel()) {
  ...
  const features = extractPullbackRegimeFeatures(bars, {
    ...
    btcFastLookbackHours:
      ...
      normalizePullbackRegimeConfig(extras.regimeCfg ?? {}).aiRegimeBtcFastLookbackHours ??
      PULLBACK_REGIME_DEFAULTS.aiRegimeBtcFastLookbackHours,
  });
...
function labelBadRegimeV1(trade) {
...
/** Bad regime: stop-loss or clear loss only; small PnL is neutral (not bad). */
function labelBadRegime(trade) {
  const pnlPct = num(trade.pnlPct, 0);
  if (trade.exitReason === "stop_loss") return 1;
  if (pnlPct < -1) return 1;
  if (pnlPct > 0.5) return 0;
  return 0;
}
...
async function buildTrainingSamples(trades, fetchBars, options = {}) {
  ...
  const labelFn = options.labelFn ?? labelBadRegime;
...
async function trainFromTrades(trades, fetchBars, options = {}) {
  ...
  const featNames = options.featureNames ?? FEATURE_NAMES;
  const modelVersion = options.modelVersion ?? (featNames.length > FEATURE_NAMES_V1.length ? 3 : 1);
