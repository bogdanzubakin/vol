...
const FEATURE_NAMES_WITH_FUNDING_OI = [...FEATURE_NAMES, ...FUNDING_OI_FEATURE_NAMES];
...
function sweepDepthPctFromMetrics(metrics = {}) {
  if (Number.isFinite(metrics.sweepDepthPct)) return metrics.sweepDepthPct;
  if (metrics.sweepLow != null && metrics.corridorLow != null) {
    const lo = num(metrics.corridorLow, 0);
    if (lo > 0) {
      return Math.abs(((lo - num(metrics.sweepLow, lo)) / lo) * 100);
    }
  }
  if (metrics.sweepHigh != null && metrics.corridorHigh != null) {
    const hi = num(metrics.corridorHigh, 0);
    if (hi > 0) {
      return Math.abs(((num(metrics.sweepHigh, hi) - hi) / hi) * 100);
    }
  }
  return 0;
}
...
function regimeMetricsFromSignal(metrics = {}) {
  ...
  const sweepDepthPct = sweepDepthPctFromMetrics(metrics);
  return {
    ...
    sweepDepthPct: sweepDepthPct > 0 ? sweepDepthPct : null,
  };
}
...
function regimeMetricsFromTrade(trade) {
  ...
  return regimeMetricsFromSignal({
    ...
    sweepLow: snap.sweepLow,
    sweepHigh: snap.sweepHigh,
    barsSinceSweep: snap.barsSinceSweep ?? trade?.barsSinceSweep,
  });
}
...
function extractSfpRegimeFeatures(bars, extras = {}) {
  const { metrics = null, tradeStats = null, btcBars = null, asOf = null } = extras;
  ...
  const sweepDepth = sweepDepthPctFromMetrics(metrics ?? {});
  ...
  const fundingOiFeat = extractFundingOiFeatures(
    extras.fundingOi ?? extras.getFundingOiAt?.(extras.symbol, asOf) ?? {},
    signalKind === "sfp_bear" ? "pullback_bear" : "pullback"
  );
  ...
  return {
    ...
    barsSinceSweepNorm:
      metrics && Number.isFinite(barsSince) ? Math.min(1, barsSince / 12) : 0,
    ...
    fundingRateNorm: fundingOiFeat.fundingRateNorm,
    fundingTrend: fundingOiFeat.fundingTrend,
    oiDelta1h: fundingOiFeat.oiDelta1h,
  };
}

function featuresToVector(features, featureNames = FEATURE_NAMES) {
  const names = Array.isArray(featureNames) && featureNames.length ? featureNames : FEATURE_NAMES;
  return names.map((name) => num(features[name], 0));
}
...
/** Incremental per-symbol SFP stats (call after each closed SFP trade in backtest). */
function recordSfpTradeStats(map, trade) {
  if (!map || !trade) return map;
  const kind = trade.signalKind;
  if (kind !== "sfp" && kind !== "sfp_bear") return map;
