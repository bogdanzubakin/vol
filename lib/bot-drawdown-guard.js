...
function closedTradesRealizedPnl(closedTrades = [], options = {}) {
  const normalizeLeverage = options.normalizeLeverage === true;
  return (closedTrades ?? []).reduce((s, t) => {
    const pnl = Number(t.pnl) || 0;
    if (!normalizeLeverage) return s + pnl;
    const lev = Math.max(1, Number(t.leverage) || 1);
    return s + pnl / lev;
  }, 0);
}

function drawdownEquityFromClosed(baseline, closedTrades = [], options = {}) {
  const ref = Number(baseline);
  if (!Number.isFinite(ref) || ref <= 0) return null;
  return ref + closedTradesRealizedPnl(closedTrades, options);
}
...
function formatDrawdownTelegramMessage(botLabel, payload) {
  const lines = [
    ...
    `Baseline $${payload.baseline.toFixed(2)} → closed trades $${payload.equity.toFixed(2)}`,
