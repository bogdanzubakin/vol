...
function normalizeDrawdownConfig(raw = {}) {
  return {
    drawdownStopEnabled: Boolean(raw.drawdownStopEnabled),
  ...
  return {
    ...
    remainingPct:
      baseline != null && cfg.drawdownStopEnabled
        ? +Math.max(0, (cfg.drawdownStopPct ?? 10) - lossPct).toFixed(2)
        : null,
    ...
    triggeredAtIso: state.drawdownTriggeredAt
      ...
      : null,
    active: Boolean(
      cfg.drawdownStopEnabled && cfg.enabled && !state.drawdownTriggeredAt
    ),
  };
}
...
function formatDrawdownTelegramMessage(botLabel, payload) {
  const lines = [
    ...
    `Baseline $${payload.baseline.toFixed(2)} → now $${payload.equity.toFixed(2)}`,
...
function evaluateDrawdownStop(cfg, state, equity, options = {}) {
  ...
  if (!cfg.drawdownStopEnabled || !cfg.enabled) return null;
