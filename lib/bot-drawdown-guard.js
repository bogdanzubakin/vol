...
function closedTradesRealizedPnl(closedTrades = []) {
  return (closedTrades ?? []).reduce((s, t) => s + (t.pnl ?? 0), 0);
}

function drawdownEquityFromClosed(baseline, closedTrades = []) {
  const ref = Number(baseline);
  if (!Number.isFinite(ref) || ref <= 0) return null;
  return ref + closedTradesRealizedPnl(closedTrades);
}
