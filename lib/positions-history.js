...
function createPositionsHistoryStore(options = {}) {
  ...
  async function list(searchParams) {
    ...
    try {
      ...
      const reconstructedBySymbol = new Map();
      for (const r of reconstructed) {
        const sym = String(r.symbol || "").toUpperCase();
        if (!reconstructedBySymbol.has(sym)) reconstructedBySymbol.set(sym, []);
        reconstructedBySymbol.get(sym).push(closeMs(r));
      }
      const incomeFallback = incomeItems.filter((x) => {
        if (seen.has(toMinuteKey(x.symbol, x.closedAt, x.profit))) return false;
        const sym = String(x.symbol || "").toUpperCase();
        const t = closeMs(x);
        const nearby = reconstructedBySymbol.get(sym) || [];
        // If we already reconstructed a close for same symbol around this time,
        // suppress income row to avoid scale-in/out duplicates.
        if (nearby.some((rt) => Math.abs(rt - t) <= 2 * 60 * 1000)) return false;
        return true;
      });
