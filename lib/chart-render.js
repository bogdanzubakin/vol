...
function sliceWindow(bars, cfg, windowBars) {
...
function buildChartConfig(symbol, slice, analysis, cfg, meta = {}) {
  ...
  const volMult = cfg.minBreakVolumeMultiplier ?? 2;
...
function getChartPayload(symbol, bars, cfg, analysis, opts = {}) {
  const slice = sliceWindow(bars, cfg, opts.windowBars ?? DEFAULTS.windowBars);
  const meta = { evaluateBarAt: opts.evaluateBarAt ?? null };
  return {
    symbol,
