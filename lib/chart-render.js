...
function renderChartToBuffer(chartConfig, width, height, extraPlugins = [], opts = {}) {
  ensureChartFonts();
  const { ChartJSNodeCanvas } = require("chartjs-node-canvas");
  const annotationPlugin = require("chartjs-plugin-annotation");
  const canvas = new ChartJSNodeCanvas({
    ...
    height,
    backgroundColour: "#0f172a",
    plugins: { modern: [annotationPlugin, ...extraPlugins] },
    chartCallback: (ChartJS) => configureChartJs(ChartJS),
  });
  const out = opts.skipDisplay ? chartConfig : applyChartDisplay(chartConfig);
  return canvas.renderToBuffer(out, "image/png");
}
...
function sanitizeAnnotations(annotations) {
  ...
  for (const [key, ann] of Object.entries(annotations || {})) {
    if (!ann) continue;
    const copy = { ...ann };
    if (copy.label == null) delete copy.label;
    else if (copy.label.display !== false) {
      copy.label = { borderWidth: 0, padding: 4, ...copy.label };
    } else {
      delete copy.label;
    }
    out[key] = copy;
