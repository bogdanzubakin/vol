...
function buildPaperBotTradeChartConfig(symbol, bars, trade, cfg = {}) {
  ...
  const annotations = {
    ...
    details: {
      type: "label",
      xValue: Math.max(0, Math.min(8, labels.length - 1)),
      yValue: yMin + span * 0.08,
      xScaleID: "x",
      yScaleID: "yPrice",
      content: detailLines,
      color: "#e2e8f0",
      font: { size: 11, family: serverFontFamily() },
      textAlign: "left",
      xAdjust: -Math.round(snapshotWidth * 0.2),
  ...
  return {
    ...
    options: {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: `${symbol} ${interval} · ${corridorDays}d corridor${logNote} · ${reason} · PnL ${pnlSign}$${Number(trade.pnl).toFixed(2)}`,
          color: (trade.pnl ?? 0) >= 0 ? "#86efac" : "#fca5a5",
          font: { size: 14, family: serverFontFamily() },
        },
...
async function renderPaperBotTradeChart(symbol, bars, trade, cfg = {}, opts = {}) {
  const config = applyServerChartFonts(
    buildPaperBotTradeChartConfig(symbol, bars, trade, cfg)
  );
  const r = getRenderer(opts);
