...
function buildPaperBotTradeChartConfig(symbol, bars, trade, cfg = {}) {
  ...
  const entryIdx = findBarIndex(bars, trade.openedAt);
  const exitIdx = findBarIndex(bars, trade.closedAt);
  ...
  const detailLines = [
    ...
    `PnL ${pnlSign}$${Number(trade.pnl).toFixed(2)} (${Number(trade.pnlPct).toFixed(2)}%)`,
    `SL ${Number(trade.stopLoss).toFixed(6)} · TP ${Number(trade.takeProfit).toFixed(6)}`,
    `Corridor ${Number(corridorLow).toFixed(6)} – ${Number(corridorHigh).toFixed(6)}`,
  ...
  const annotations = {
    corridorBox:
      corridorHigh != null && corridorLow != null
        ? {
            type: "box",
            xMin: 0,
    ...
    exitLine: {
      ...
      yScaleID: "yPrice",
      borderColor: (trade.pnl ?? 0) >= 0 ? "#22c55e" : "#ef4444",
      borderWidth: 2,
    },
    entryMark:
      entryIdx >= 0
      ...
      exitIdx >= 0
        ? {
            ...
            label: {
              display: true,
              content: reason,
              color: (trade.pnl ?? 0) >= 0 ? "#86efac" : "#fca5a5",
              backgroundColor: "rgba(15,23,42,0.85)",
            },
          }
        : undefined,
    details: {
      type: "label",
      xValue: Math.max(0, labels.length - 1),
      yValue: yMax - span * 0.05,
      xScaleID: "x",
      yScaleID: "yPrice",
      content: detailLines,
      color: "#e2e8f0",
      ...
      backgroundColor: "rgba(15,23,42,0.92)",
      borderColor:
        (trade.pnl ?? 0) >= 0 ? "rgba(34,197,94,0.6)" : "rgba(239,68,68,0.5)",
      borderWidth: 1,
      borderRadius: 6,
      padding: 10,
    },
  ...
  return {
    ...
    options: {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: `${symbol} ${interval} · ${reason} · PnL ${pnlSign}$${Number(trade.pnl).toFixed(2)}`,
          color: (trade.pnl ?? 0) >= 0 ? "#86efac" : "#fca5a5",
          font: { size: 14 },
        },
        annotation: { annotations },
...
async function renderPaperBotTradeChart(symbol, bars, trade, cfg = {}, opts = {}) {
  const config = buildPaperBotTradeChartConfig(symbol, bars, trade, cfg);
  const r = getRenderer(opts);
  return r.renderToBuffer(applyChartDisplay(config), "image/png");
}
