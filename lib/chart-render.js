...
const FOCUSED_THEMES = {
  ...
  sfp_bear: {
    label: "SFP↓ · sweep-reject",
    accent: "#f87171",
    preFill: "rgba(248,113,113,0.1)",
    posFill: "rgba(248,113,113,0.05)",
    sweep: "#fb923c",
    sweepFill: "rgba(251,146,60,0.15)",
  },
  pullback: {
  ...
  pullback_bear: {
    label: "PB↓ · MA rejection",
    accent: "#fdba74",
    preFill: "rgba(251,146,60,0.1)",
    posFill: "rgba(251,146,60,0.05)",
    ma: "#38bdf8",
  },
  foi: {
    label: "FOI · funding unwind long",
    accent: "#34d399",
    preFill: "rgba(52,211,153,0.1)",
    posFill: "rgba(52,211,153,0.05)",
    sweep: "#2dd4bf",
    sweepFill: "rgba(45,212,191,0.15)",
  },
  foi_bear: {
    label: "FOI↓ · funding unwind short",
    accent: "#fb7185",
    preFill: "rgba(251,113,133,0.1)",
    posFill: "rgba(251,113,133,0.05)",
    sweep: "#f472b6",
    sweepFill: "rgba(244,114,182,0.15)",
  },
};
...
function focusedDetailLines(trade, meta, theme) {
  ...
  if (
    (meta.chartTheme === "pullback" || meta.chartTheme === "pullback_bear") &&
    meta.maBars
  ) {
    lines.push(
  ...
  if (meta.chartTheme === "sfp" || meta.chartTheme === "sfp_bear") {
    if (meta.sweepLow != null) {
      lines.push(`Sweep low ${Number(meta.sweepLow).toFixed(6)}`);
    }
    if (meta.sweepHigh != null) {
      lines.push(`Sweep high ${Number(meta.sweepHigh).toFixed(6)}`);
    }
    if (meta.reclaimLevel != null) {
      lines.push(`Reclaim ${Number(meta.reclaimLevel).toFixed(6)}`);
    }
    if (meta.rejectLevel != null) {
      lines.push(`Reject ${Number(meta.rejectLevel).toFixed(6)}`);
    }
  }
...
function drawPaperBotFocusedChart(canvas, symbol, bars, trade, meta, cfg = {}) {
  ...
  ctx.font = `11px ${font}`;
  ctx.fillText(
    `${meta.preEntryMinutes ?? 30}m before entry + position` +
      (isOpen ? "" : ` + ${meta.postExitHours ?? 6}h after`),
    16,
    46
  );
  ...
  if (meta.chartTheme === "sfp" || meta.chartTheme === "sfp_bear") {
    if (meta.chartTheme === "sfp_bear" || meta.bear) {
      drawHLine(meta.sweepHigh, theme.sweep, [], 2);
      drawHLine(meta.sweepThreshold, theme.sweep, [3, 3], 1);
      drawHLine(meta.rejectLevel, "#22d3ee", [4, 3], 1);
    } else {
  ...
  if (
    (meta.chartTheme === "sfp_bear" || meta.bear) &&
    Number.isFinite(meta.sweepHigh)
  ) {
    let sweepIdx = -1;
    for (let i = 0; i <= Math.min(entryIdx, bars.length - 1); i++) {
      if (bars[i].high >= meta.sweepHigh * 0.999) {
        sweepIdx = i;
        break;
      }
    }
    if (sweepIdx >= 0) {
      ...
      ctx.fillStyle = theme.sweep;
      ctx.beginPath();
      ctx.moveTo(sx, toY(bars[sweepIdx].high) - 10);
      ctx.lineTo(sx - 5, toY(bars[sweepIdx].high) - 18);
      ctx.lineTo(sx + 5, toY(bars[sweepIdx].high) - 18);
      ctx.closePath();
      ctx.fill();
    }
