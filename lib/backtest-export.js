...
function computeAnalytics(trades, summary, perSymbol, result) {
  ...
  const sameBarExits = trades.filter((t) => t.sameBarExit).length;
  ...
  return {
    ...
    sameBarExitCount: sameBarExits,
    sameBarExitNote:
      sameBarExits > 0
        ? "openedAt equals closedAt (entry and exit on same bar timestamp). " +
          "Usually means the backtest applied SL/TP on the entry bar; re-run after fix to defer exits to the next bar."
        : null,
    ...
    sfpRegimeSkips: result?.summary?.sfpRegimeSkips != null
      ? {
          total: result.summary.sfpRegimeSkips,
          bull: result.summary.sfpRegimeSkipsBull ?? 0,
          bear: result.summary.sfpRegimeSkipsBear ?? 0,
          enabled: Boolean(result.botConfig?.aiSfpRegimeEnabled),
          bullThreshold:
            result.botConfig?.aiSfpRegimeBullThreshold ??
            result.botConfig?.aiSfpRegimeThreshold,
          bearThreshold:
            result.botConfig?.aiSfpRegimeBearThreshold ??
            result.botConfig?.aiSfpRegimeThreshold,
        }
      : null,
...
function tradeExportIntegrity(trades, summary, result) {
  const stored = trades.length;
  const total = result?.closedTradesTotal ?? stored;
  const pnlSum = +trades.reduce((s, t) => s + (t.pnl ?? 0), 0).toFixed(4);
  const realized = summary?.realizedPnl ?? summary?.totalPnl ?? null;
  const wins = trades.filter((t) => (t.pnl ?? 0) > 0).length;
  const losses = trades.filter((t) => (t.pnl ?? 0) < 0).length;
  const truncated = total > stored;
  const pnlMismatch =
    realized != null && Math.abs(pnlSum - realized) > 0.02;
  const winLossMismatch =
    summary &&
    (wins !== summary.winCount || losses !== summary.lossCount);
  const issues = [];
  if (truncated) {
    issues.push(
      `Only ${stored} of ${total} closed trades are stored — summary and analytics will not match the full run. Re-run train bot on the latest scanner build.`
    );
  }
  if (pnlMismatch) {
    issues.push(
      `Sum of exported trade PnL (${pnlSum}) does not match summary realized/total PnL (${realized}).`
    );
  }
  if (winLossMismatch) {
    issues.push(
      `Exported win/loss counts (${wins}/${losses}) differ from summary (${summary.winCount}/${summary.lossCount}).`
    );
  }
  return {
    ok: issues.length === 0,
    storedTradeCount: stored,
    totalTradeCount: total,
    truncated,
    exportedPnlSum: pnlSum,
    summaryRealizedPnl: realized,
    exportedWins: wins,
    exportedLosses: losses,
    summaryWins: summary?.winCount ?? null,
    summaryLosses: summary?.lossCount ?? null,
    issues,
  };
}

function summarizeRegimeSkips(events, summary = {}) {
  const list = events ?? [];
  const regimeEvents = list.filter(
    (e) =>
      e.type === "SKIP" &&
      String(e.detail ?? "").toLowerCase().includes("sfp regime")
  );
  const bull = regimeEvents.filter((e) =>
    String(e.detail ?? "").toLowerCase().includes("bull")
  ).length;
  const bear = regimeEvents.filter((e) =>
    String(e.detail ?? "").toLowerCase().includes("bear")
  ).length;
  const unknown = regimeEvents.length - bull - bear;

  return {
    total: summary.sfpRegimeSkips ?? regimeEvents.length,
    bull: summary.sfpRegimeSkipsBull ?? bull,
    bear: summary.sfpRegimeSkipsBear ?? bear,
    unknown,
    fromSummary: Boolean(summary.sfpRegimeSkips != null),
    samples: regimeEvents.slice(0, 20).map((e) => ({
      symbol: e.symbol,
      detail: e.detail,
      at: e.at,
    })),
...
function summarizeEvents(events, summary = {}) {
  const list = events ?? [];
  ...
  const skipDetails = {};
  for (const e of list.filter((ev) => ev.type === "SKIP")) {
    const d = String(e.detail ?? "unknown").slice(0, 96);
    skipDetails[d] = (skipDetails[d] ?? 0) + 1;
  }
  const topSkipReasons = Object.entries(skipDetails)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 15)
  ...
  return {
    ...
    sfpRegimeSkips: summarizeRegimeSkips(list, summary),
    topSkipReasons,
  };
}
...
function buildBacktestExport(options = {}) {
  ...
  const integrity = tradeExportIntegrity(trades, result.summary, result);

  const bundle = {
    exportFormatVersion: EXPORT_FORMAT_VERSION,
    readme:
      "Train bot optimisation export (v2). Upload to an AI assistant for parameter tuning. " +
      "Includes enriched per-trade metrics (MFE/MAE, signal snapshot at entry, corridor/SL/TP distances, exitPathOracle for SFP AI training), " +
      "equity curve, event log summary (incl. SFP regime skip breakdown), cross-tabs (exit reason × add-ons), histograms, daily PnL, and per-symbol breakdown.",
  ...
  if (includeEvents && result.events?.length) {
    bundle.events = {
      summary: summarizeEvents(result.events, result.summary ?? {}),
      log: result.events,
    };
  } else if (result.summary?.sfpRegimeSkips != null) {
    bundle.events = {
      summary: summarizeEvents([], result.summary),
    };
  }
