...
} = require("./level-break-signal-features");
...
const MODEL_BASENAME = "level-break-signal-model";
const MODEL_FILE = (scope = "paper") => modelFileFor(MODEL_BASENAME, scope);

const yieldToLoop = () => new Promise((resolve) => setImmediate(resolve));

const LEVEL_BREAK_SIGNAL_DEFAULTS = {
  aiLevelBreakSignalEnabled: false,
  aiLevelBreakSignalThreshold: 0.52,
  aiLevelBreakSignalBullThreshold: 0.5,
  aiLevelBreakSignalBearThreshold: 0.54,
  aiLevelBreakSignalBtcLookbackHours: 12,
};
...
const DEFAULT_MODEL = {
  ...
  bull: bootstrapSubModel(
    ...
    -0.25,
    LEVEL_BREAK_SIGNAL_DEFAULTS.aiLevelBreakSignalBullThreshold
  ),
  bear: bootstrapSubModel(
    BOOTSTRAP_WEIGHTS_BEAR,
    -0.2,
    LEVEL_BREAK_SIGNAL_DEFAULTS.aiLevelBreakSignalBearThreshold
  ),
  trainedAt: null,
  source: "bootstrap",
};
...
function normalizeLevelBreakSignalConfig(raw = {}) {
  const legacy = clamp(
    num(raw.aiLevelBreakSignalThreshold, LEVEL_BREAK_SIGNAL_DEFAULTS.aiLevelBreakSignalThreshold),
    0.35,
    0.9
  );
  const bullTh = clamp(num(raw.aiLevelBreakSignalBullThreshold, legacy), 0.35, 0.9);
  const bearTh = clamp(num(raw.aiLevelBreakSignalBearThreshold, legacy), 0.35, 0.9);
  return {
    aiLevelBreakSignalEnabled: Boolean(raw.aiLevelBreakSignalEnabled),
    aiLevelBreakSignalThreshold: legacy,
    aiLevelBreakSignalBullThreshold: bullTh,
    aiLevelBreakSignalBearThreshold: bearTh,
    aiLevelBreakSignalBtcLookbackHours: clamp(
      Math.round(
        num(
          raw.aiLevelBreakSignalBtcLookbackHours,
          LEVEL_BREAK_SIGNAL_DEFAULTS.aiLevelBreakSignalBtcLookbackHours
        )
      ),
      1,
      72
...
function thresholdForSignal(signalCfg, signalKind, subModel) {
  const c = normalizeLevelBreakSignalConfig(signalCfg);
  if (isBearSignal(signalKind)) {
    return c.aiLevelBreakSignalBearThreshold ?? subModel?.threshold ?? 0.54;
  }
  return c.aiLevelBreakSignalBullThreshold ?? subModel?.threshold ?? 0.5;
}
...
function predictLevelBreakSignal(bars, extras = {}, model = getModel()) {
  ...
  const metrics = extras.metrics
    ...
    : null;
  const features = extractLevelBreakSignalFeatures(bars, {
    metrics,
    ...
    btcLookbackHours:
      ...
      normalizeLevelBreakSignalConfig(extras.signalCfg ?? {}).aiLevelBreakSignalBtcLookbackHours ??
      LEVEL_BREAK_SIGNAL_DEFAULTS.aiLevelBreakSignalBtcLookbackHours,
  });
...
function evaluateLevelBreakSignalGate(cfg, bars, extras = {}) {
  const signalCfg = normalizeLevelBreakSignalConfig(cfg);
  if (!signalCfg.aiLevelBreakSignalEnabled) return { pass: true, enabled: false };
  ...
  const metrics = extras.metrics
    ? regimeMetricsFromSignal(extras.metrics, signalKind)
    : null;
  const { probability, features, head } = predictLevelBreakSignal(
    bars,
    {
      ...extras,
      ...
      modelScope,
      signalCfg,
      btcLookbackHours: signalCfg.aiLevelBreakSignalBtcLookbackHours,
    },
    stored
  );
  ...
  const detail = `weak ${side} level-break signal p=${(probability * 100).toFixed(0)}% < ${(threshold * 100).toFixed(0)}% · break ${(
...
async function buildTrainingSamples(trades, fetchBars, options = {}) {
  const samples = [];
  const cfg = normalizeLevelBreakSignalConfig(options);
  ...
  for (let i = 0; i < total; i++) {
    ...
    const features = extractLevelBreakSignalFeatures(window, {
      metrics: regimeMetricsFromTrade(trade),
      tradeStats,
      signalKind: trade.signalKind,
      btcBars,
      asOf: trade.openedAt,
      btcLookbackHours: cfg.aiLevelBreakSignalBtcLookbackHours,
    });
    samples.push({
...
async function trainFromTrades(trades, fetchBars, options = {}) {
  ...
  if (bullPos.length < 4 && bearPos.length < 4) {
    throw new Error(
      `Too few winning level-break labels (bull ${bullPos.length}, bear ${bearPos.length})`
    );
  }
  ...
  const bullModel =
    bullTrain?.length >= 12
      ? trainLogisticRegression(bullTrain, {
          head: "bull",
          threshold: cfg.aiLevelBreakSignalBullThreshold,
  ...
  const bearModel =
    bearTrain?.length >= 12
      ? trainLogisticRegression(bearTrain, {
          head: "bear",
          threshold: cfg.aiLevelBreakSignalBearThreshold,
          source: options.source ?? "trained",
          onProgress,
        })
      : { ...DEFAULT_MODEL.bear };
...
function getModelStatus(scope = "paper") {
  ...
  return {
    ...
    defaults: LEVEL_BREAK_SIGNAL_DEFAULTS,
  };
}
...
module.exports = {
  LEVEL_BREAK_SIGNAL_DEFAULTS,
  MODEL_FILE,
  isBearSignal,
  normalizeLevelBreakSignalConfig,
  getModel,
  reloadModel,
  saveModel,
  predictLevelBreakSignal,
  evaluateLevelBreakSignalGate,
