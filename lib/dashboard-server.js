...
const { nowIsoUtcPlus3 } = require("./time-format");
...
function startDashboard(getState, options = {}) {
  ...
  const { onConfigUpdate, getPairs, getChartData, postTelegramSignal } =
    options;
...
function createDashboardPublisher(cfg, options = {}) {
  ...
  function buildState(activeHits, nearBreakHits = new Map()) {
    const hits = [...activeHits.entries()]
      .map(([symbol, m]) => ({ symbol, signalStatus: "active", ...m }))
  ...
  function publish(activeHits, nearBreakHits, force = false) {
    const state = buildState(activeHits, nearBreakHits);
