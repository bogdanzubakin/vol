...
const {
  ...
  exportFilename: liveHistoryExportFilename,
} = require("./live-bot-history-export");
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === "/api/live-bot/reset-history" && req.method === "POST") {
      ...
      try {
        ...
        const result = await resetLiveBotHistory({ full: Boolean(body?.full) });
        res.writeHead(200, {
          "Content-Type": "application/json",
          "Cache-Control": "no-store",
        });
    ...
    if (url.pathname === "/api/live-bot/history" && req.method === "GET") {
      if (!getLiveBotHistory) {
        res.writeHead(503, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: "Live bot history requires the live scanner" }));
        return;
      }
      try {
        const body = JSON.stringify(await getLiveBotHistory(url.searchParams));
        res.writeHead(200, {
          "Content-Type": "application/json",
          "Cache-Control": "no-store",
        });
    ...
    if (
      url.pathname === "/api/live-bot/history/export" &&
      req.method === "GET"
    ) {
      if (!getLiveBotHistoryExport) {
        res.writeHead(503, { "Content-Type": "application/json" });
        res.end(
          JSON.stringify({
            error: "Live bot history export requires the live scanner",
          })
        );
        return;
      }
      try {
        ...
        const bundle = await getLiveBotHistoryExport(url.searchParams, {
          includeSourceCode: !flagOff("includeSource"),
          includeEquityCurve: !flagOff("includeEquityCurve"),
          includeSnapshots: flagOn("includeSnapshots"),
        });
        const json = JSON.stringify(bundle, null, 2);
        const filename = liveHistoryExportFilename();
        res.writeHead(200, {
          "Content-Type": "application/json; charset=utf-8",
          "Content-Disposition": `attachment; filename="${filename}"`,
          "Cache-Control": "no-store",
