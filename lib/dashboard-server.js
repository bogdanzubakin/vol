  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === "/api/early-exit-model" && req.method === "GET") {
      ...
      try {
        ...
        res.end(JSON.stringify(getEarlyExitModelStatus(scope)));
      } catch (e) {
        res.writeHead(500, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: e.message || String(e) }));
      }
    ...
    if (url.pathname === "/api/sfp-regime-model" && req.method === "GET") {
      ...
      try {
        ...
        res.end(JSON.stringify(getSfpRegimeModelStatus(scope)));
      } catch (e) {
        res.writeHead(500, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: e.message || String(e) }));
      }
    ...
    if (url.pathname === "/api/level-break-regime-model" && req.method === "GET") {
      ...
      try {
        ...
        res.end(JSON.stringify(getLevelBreakRegimeModelStatus(scope)));
      } catch (e) {
        res.writeHead(500, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: e.message || String(e) }));
      }
