...
function startDashboard(getState, options = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === "/api/fast-corridor") {
      if (!getFastCorridor) {
        res.writeHead(503, { "Content-Type": "application/json" });
        res.end(
          JSON.stringify({
            error: "Fast corridor list requires the live scanner",
          })
        );
        return;
      }
      try {
        const body = JSON.stringify(getFastCorridor(url.searchParams));
        res.writeHead(200, {
          "Content-Type": "application/json",
          "Cache-Control": "no-store",
        });
    ...
    if (
      ...
      try {
        const includeSource =
          url.searchParams.get("includeSource") !== "0" &&
          url.searchParams.get("includeSource") !== "false";
        const bundle = buildBacktestExport({ includeSourceCode: includeSource });
...
function createDashboardPublisher(cfg, options = {}) {
  ...
  function buildState(
    ...
    const spikeHits = buildSignalHistoryHits(
      signalHistory,
      activeHits,
      "spike"
    );
    const fcHits = buildSignalHistoryHits(
      fcHistory,
      fcActive,
      "fast-corridor"
    );
    const hits = [...spikeHits, ...fcHits].sort(
    ...
    return {
      ...
      ...meta,
      activeCount: activeHits.size + fcActive.size,
