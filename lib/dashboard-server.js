...
function startDashboard(getState, options = {}) {
  ...
  const {
    ...
    getLevelBreakRegimeModelStatus,
    getLevelBreakRegimeModelData,
    getLevelBreakRegimeMonitor,
    trainLevelBreakRegimeModel,
    importLevelBreakRegimeModel,
    getLevelBreakSignalModelStatus,
    getLevelBreakSignalModelData,
    trainLevelBreakSignalModel,
    importLevelBreakSignalModel,
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === "/api/level-break-signal-model" && req.method === "GET") {
      if (!getLevelBreakSignalModelStatus) {
        res.writeHead(503, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: "Level-break signal model unavailable" }));
        return;
      }
      try {
        const scope = url.searchParams.get("scope");
        const full = url.searchParams.get("full") === "1";
        const payload = { ...getLevelBreakSignalModelStatus(scope) };
        if (full && getLevelBreakSignalModelData) {
          payload.model = getLevelBreakSignalModelData(scope);
        }
        res.writeHead(200, {
          "Content-Type": "application/json",
          "Cache-Control": "no-store",
    ...
    if (
      url.pathname === "/api/level-break-signal-model/train" &&
      req.method === "POST"
    ) {
      if (!trainLevelBreakSignalModel) {
        res.writeHead(503, { "Content-Type": "application/json" });
        res.end(
          JSON.stringify({ error: "Level-break signal training unavailable" })
        );
        return;
      }
      try {
        const body = await readJsonBody(req);
        const result = trainLevelBreakSignalModel(body);
        res.writeHead(200, {
          "Content-Type": "application/json",
          "Cache-Control": "no-store",
        });
    ...
    if (url.pathname === "/api/level-break-signal-model" && req.method === "PUT") {
      if (!importLevelBreakSignalModel) {
        res.writeHead(503, { "Content-Type": "application/json" });
        res.end(
          JSON.stringify({ error: "Level-break signal model import unavailable" })
        );
        return;
      }
      try {
        const body = await readJsonBody(req);
        const result = importLevelBreakSignalModel(body);
        res.writeHead(200, {
          "Content-Type": "application/json",
          "Cache-Control": "no-store",
        });
    ...
    if (url.pathname === "/api/level-break-regime-model" && req.method === "GET") {
      if (!getLevelBreakRegimeModelStatus) {
        res.writeHead(503, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: "Level-break regime model unavailable" }));
        return;
      }
      try {
        const scope = url.searchParams.get("scope");
        const full = url.searchParams.get("full") === "1";
        const payload = { ...getLevelBreakRegimeModelStatus(scope) };
        if (full && getLevelBreakRegimeModelData) {
          payload.model = getLevelBreakRegimeModelData(scope);
        }
        res.writeHead(200, {
          "Content-Type": "application/json",
          "Cache-Control": "no-store",
    ...
    if (url.pathname === "/api/level-break-regime-monitor" && req.method === "GET") {
      if (!getLevelBreakRegimeMonitor) {
        res.writeHead(503, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: "Level-break regime monitor unavailable" }));
        return;
      }
      try {
        const scope = url.searchParams.get("scope");
        ...
        res.end(JSON.stringify(getLevelBreakRegimeMonitor(scope)));
      } catch (e) {
        res.writeHead(500, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: e.message || String(e) }));
      }
    ...
    if (
      url.pathname === "/api/level-break-regime-model/train" &&
      req.method === "POST"
    ) {
      if (!trainLevelBreakRegimeModel) {
        res.writeHead(503, { "Content-Type": "application/json" });
        res.end(
          JSON.stringify({ error: "Level-break regime training unavailable" })
        );
        return;
      }
      try {
        const body = await readJsonBody(req);
        const result = trainLevelBreakRegimeModel(body);
        res.writeHead(200, {
          "Content-Type": "application/json",
          "Cache-Control": "no-store",
        });
    ...
    if (url.pathname === "/api/level-break-regime-model" && req.method === "PUT") {
      if (!importLevelBreakRegimeModel) {
        res.writeHead(503, { "Content-Type": "application/json" });
        res.end(
          JSON.stringify({ error: "Level-break regime model import unavailable" })
        );
        return;
      }
      try {
        const body = await readJsonBody(req);
        const result = importLevelBreakRegimeModel(body);
        res.writeHead(200, {
          "Content-Type": "application/json",
          "Cache-Control": "no-store",
        });
