...
function createFuturesTrader(options = {}) {
  ...
  let hedgeModeCache = { at: 0, hedge: null, inflight: null };
  ...
  function invalidateRestCache() {
    ...
    hedgeModeCache = { at: 0, hedge: null, inflight: null };
  }

  async function getHedgeMode() {
    const now = Date.now();
    if (hedgeModeCache.hedge != null && now - hedgeModeCache.at < 60_000) {
      return hedgeModeCache.hedge;
    }
    if (hedgeModeCache.inflight) return hedgeModeCache.inflight;
    hedgeModeCache.inflight = (async () => {
      ...
      const hedge = Boolean(body?.dualSidePosition);
      hedgeModeCache = { at: Date.now(), hedge, inflight: null };
      return hedge;
    })().catch((e) => {
      hedgeModeCache.inflight = null;
      throw e;
    });
    return hedgeModeCache.inflight;
  }
  ...
  function parseWsPositionRow(row) {
    ...
    const existingPos = pickPosition(existing);
    return {
      symbol: sym,
      positionAmt: amt,
      positionSide: String(
        row.ps ?? row.positionSide ?? existingPos?.positionSide ?? "BOTH"
      ).toUpperCase(),
      entryPrice: Number(row.ep ?? row.entryPrice),
      markPrice: Number(row.mp ?? row.markPrice ?? existingPos?.markPrice ?? row.ep),
      ...
      liquidationPrice: Number(row.lp ?? row.liquidationPrice ?? existingPos?.liquidationPrice),
      updateTime: Number(row.uT ?? row.updateTime) || existingPos?.updateTime || null,
    };
  }
  ...
  function pickPosition(entry, sideHint = null) {
    if (!entry) return null;
    const list = Array.isArray(entry) ? entry : [entry];
    const open = list.filter(
      (p) => Number.isFinite(p.positionAmt) && Math.abs(p.positionAmt) > 1e-12
    );
    if (!open.length) return null;
    if (!sideHint) return open[0];
    const want = String(sideHint).toUpperCase();
    if (want === "LONG" || want === "SHORT") {
      const byPs = open.find((p) => p.positionSide === want);
      if (byPs) return byPs;
      if (want === "SHORT") return open.find((p) => p.positionAmt < 0) ?? null;
      return open.find((p) => p.positionAmt > 0) ?? null;
    }
    return open[0];
  }

  function resolveCloseFromExchange(exPos) {
    const amt = Number(exPos?.positionAmt);
    const ps = String(exPos?.positionSide || "BOTH").toUpperCase();
    if (!Number.isFinite(amt) || Math.abs(amt) < 1e-12) return null;
    const qty = Math.abs(amt);
    if (ps === "SHORT") return { side: "BUY", qty, positionSide: "SHORT" };
    if (ps === "LONG") return { side: "SELL", qty, positionSide: "LONG" };
    return {
      side: amt > 0 ? "SELL" : "BUY",
      qty,
  ...
  function isReduceOnlyRejectedError(err) {
    return (
      err?.code === -2022 ||
      /ReduceOnly Order is rejected/i.test(String(err?.message || ""))
    );
  }
  ...
  async function fetchAllPositions() {
    ...
    for (const row of rows || []) {
      ...
      if (!pos) continue;
      const cur = map.get(pos.symbol);
      if (!cur) map.set(pos.symbol, pos);
      else if (Array.isArray(cur)) cur.push(pos);
      else map.set(pos.symbol, [cur, pos]);
    }
    return map;
  }
  ...
  async function getPosition(symbol, { force = false, sideHint = null } = {}) {
    const map = await getPositionMap({ force });
    return pickPosition(map.get(symbol), sideHint);
  }
  ...
  async function marketOrder(symbol, side, quantity, opts = {}) {
    ...
    const hedge = await getHedgeMode();
    if (opts.reduceOnly) {
      if (hedge) {
        const ps =
          opts.positionSide ||
    ...
    } else if (hedge && opts.positionSide) {
  ...
  async function closeMarketPosition(symbol, opts = {}) {
    const { sideHint = null, markPrice = null } = opts;
    const exPos = await getPosition(symbol, { force: true, sideHint });
    if (!exPos) {
      return { closed: false, reason: "already_flat", exPos: null };
    }
    const close = resolveCloseFromExchange(exPos);
    if (!close) {
      return { closed: false, reason: "already_flat", exPos };
    }
    try {
      const order = await marketOrder(symbol, close.side, close.qty, {
        reduceOnly: true,
        positionSide: close.positionSide,
        markPrice: markPrice ?? exPos.markPrice,
      });
      return { closed: true, order, exPos };
    } catch (e) {
      if (isReduceOnlyRejectedError(e)) {
        return { closed: false, reason: "reduce_only_rejected", exPos, error: e };
      }
      throw e;
    }
  }

  async function closePosition(symbol, opts = {}) {
    await requireCreds();
    try {
    ...
    const result = await closeMarketPosition(symbol, opts);
    if (!result.closed) {
  ...
  async function getUserTrades(symbol, { startTime, endTime, limit = 1000 } = {}) {
    const { apiKey, apiSecret } = await requireCreds();
    ...
    return signedRequest("GET", "/fapi/v1/userTrades", params, apiKey, apiSecret);
  }
