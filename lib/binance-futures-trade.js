...
function createFuturesTrader(options = {}) {
  ...
  async function placeAlgoCloseOrder(symbol, type, triggerPrice) {
    const meta = await getSymbolMeta(symbol);
    const tp = formatPrice(triggerPrice, meta);
    if (!tp) throw new Error(`Invalid trigger price for ${symbol}`);
    const { apiKey, apiSecret } = await requireCreds();
  ...
  function isCloseStopAlgo(order) {
    const type = String(order?.orderType || order?.type || "").toUpperCase();
    if (type !== "STOP_MARKET") return false;
    const side = String(order?.side || "").toUpperCase();
    if (side && side !== "SELL") return false;
    const cp = order?.closePosition;
    return cp === true || cp === "true" || cp === "1" || side === "SELL";
  }

  function isCloseTakeProfitAlgo(order) {
    const type = String(order?.orderType || order?.type || "").toUpperCase();
  ...
  function pickCloseStopAlgo(orders) {
    return normalizeAlgoOrders(orders).find(isCloseStopAlgo) ?? null;
  }

  function pickCloseTakeProfitAlgo(orders) {
    return normalizeAlgoOrders(orders).find(isCloseTakeProfitAlgo) ?? null;
  }

  function hasCloseStopLoss(orders) {
    return pickCloseStopAlgo(orders) != null;
  }

  function hasCloseTakeProfit(orders) {
    return pickCloseTakeProfitAlgo(orders) != null;
  }

  async function getAlgoOpenOrders(symbol) {
    const { apiKey, apiSecret } = await requireCreds();
    const params = {};
    ...
    return normalizeAlgoOrders(body);
  }
  ...
  async function placeCloseStop(symbol, stopPrice) {
    return placeAlgoCloseOrder(symbol, "STOP_MARKET", stopPrice);
  }

  async function placeCloseTakeProfit(symbol, stopPrice) {
    return placeAlgoCloseOrder(symbol, "TAKE_PROFIT_MARKET", stopPrice);
  }
  ...
  async function closePosition(symbol) {
    ...
    const exPos = await getPosition(symbol);
