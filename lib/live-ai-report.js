...
function exitLevelsCard(status = {}, cfg = {}, impact = {}) {
  ...
  return {
    ...
    sourceLabel: humanizeSource(status.source),
    trainedAt: status.trainedAt ? formatTrainedAt(status.trainedAt) : null,
    training: status.training?.running
      ...
      : status.training?.error
        ...
        : null,
    metrics: {
      bullSlMae: sanitizeMaePct(bull.slMae),
      bullTpMae: sanitizeMaePct(bull.tpMae),
      bullSamples: bull.samples ?? null,
      bearSlMae: sanitizeMaePct(bear.slMae),
      bearTpMae: sanitizeMaePct(bear.tpMae),
      bearSamples: bear.samples ?? null,
    },
    bearMirrorsBull: Boolean(status.bearMirrorsBull),
