...
const POSITION_EXIT_DEFAULTS = {
  ...
  /** Sub-toggles (default ON when earlyAbortEnabled). */
  earlyAbortStallEnabled: true,
  earlyAbortInvalidationEnabled: true,
...
function normalizePositionExitConfig(raw = {}) {
  ...
  return {
    ...
    earlyAbortStallEnabled:
      raw.earlyAbortStallEnabled !== undefined
        ? Boolean(raw.earlyAbortStallEnabled)
        : d.earlyAbortStallEnabled,
    earlyAbortInvalidationEnabled:
      raw.earlyAbortInvalidationEnabled !== undefined
        ? Boolean(raw.earlyAbortInvalidationEnabled)
        : d.earlyAbortInvalidationEnabled,
    earlyAbortAdverseEnabled:
      raw.earlyAbortAdverseEnabled !== undefined
        ? Boolean(raw.earlyAbortAdverseEnabled)
        : d.earlyAbortAdverseEnabled,
...
function evaluateEarlyAbort(cfg, pos, bar) {
  ...
  const invalidationOn = cfg.earlyAbortInvalidationEnabled !== false;
  const adverseOn = cfg.earlyAbortAdverseEnabled !== false;
  const stallOn = cfg.earlyAbortStallEnabled !== false;

  if (invalidationOn && bars > 0 && bars <= invalidateBars) {
    if (isShort(pos)) {
  ...
  if (adverseOn && bars > 0 && bars <= maxBars) {
    if (
      adverse != null &&
      adverse >= maxAdv &&
