...
function takeProfitPctForSignal(signalKind, cfg) {
  ...
  if (signalKind === "sfp" && num(cfg.sfpTakeProfitPct, 0) > 0) {
...
function peakMovePct(pos) {
  ...
  return movePctFromEntry(peak, pos.initialEntryPrice ?? pos.entryPrice);
}

function troughMovePct(pos) {
  const trough = pos.troughPrice ?? pos.entryPrice;
  return movePctFromEntry(trough, pos.initialEntryPrice ?? pos.entryPrice);
}
...
function evaluateEarlyAbort(cfg, pos, bar) {
  ...
  const trough = troughMovePct(pos);
  ...
  if (bars > 0 && bars <= maxBars) {
    if (
      ...
      return {
        ...
        exitPrice: close,
        detail: `trough ${trough.toFixed(2)}%`,
      };
    }
  }
...
function countRunnerReversalSignals(cfg, pos, bar) {
  ...
  if (!Number.isFinite(close) || !Number.isFinite(peak) || peak <= 0) return 0;
...
function processRunnerPhase(cfg, pos, bar) {
  ...
  if (Number.isFinite(peak) && peak > 0) {
    ...
    if (Number.isFinite(trailSl) && trailSl > pos.stopLoss) {
