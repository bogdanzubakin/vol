...
function takeProfitPctForSignal(signalKind, cfg) {
  ...
  if (
    ...
    num(cfg.levelBreakTakeProfitPct, 0) > 0
  ) {
    return Math.min(num(cfg.levelBreakTakeProfitPct, base), base);
  }
  return base;
}
...
function seedPositionExitContext(pos, metrics) {
  ...
  if (pos.signalKind === "level_break" && Number.isFinite(pos.levelPrice)) {
    pos.reclaimLevel = pos.levelPrice;
  }
  if (pos.signalKind === "level_break_bear" && Number.isFinite(pos.levelPrice)) {
    pos.rejectLevel = pos.levelPrice;
  }
