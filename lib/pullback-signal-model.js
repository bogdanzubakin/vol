...
function predictPullbackSignal(bars, extras = {}, model = getModel()) {
  ...
  const useOnnx =
    stored.backend === "onnx" ||
    signalCfg.aiPullbackSignalOnnxEnabled;
  if (useOnnx) {
    const head = isBearSignal(signalKind) ? "bear" : "bull";
    const scope = extras.modelScope ?? "paper";
    const onnxPath =
      (head === "bear" ? stored.onnxBearPath : stored.onnxBullPath) ??
      onnxModelPathsForScope(scope, head);
    const gbm = loadGbmEnsemble(scope, head);
    if (gbm) {
      probability = predictGbmProbability(gbm, vec);
      backend = "gbm";
    } else if (isOnnxRuntimeAvailable()) {
      probability = predictOnnxProbability(onnxPath, vec);
      backend = "onnx";
    } else {
