...
const { applyBarConfig, sweepReclaimMetrics, sweepRejectMetrics, smaClose } = require("./signal-metrics");
...
const SNAPSHOT_POST_EXIT_HOURS = 6;

function snapshotPostExitMs(cfg = {}) {
  if (cfg.snapshotPostExitHours != null && Number.isFinite(Number(cfg.snapshotPostExitHours))) {
    return Number(cfg.snapshotPostExitHours) * 60 * 60 * 1000;
  }
  if (cfg.snapshotPostExitMinutes != null && Number.isFinite(Number(cfg.snapshotPostExitMinutes))) {
    return Number(cfg.snapshotPostExitMinutes) * 60 * 1000;
  }
  return SNAPSHOT_POST_EXIT_HOURS * 60 * 60 * 1000;
}
...
function snapshotChartFamily(signalKind) {
  if (signalKind === "sfp" || signalKind === "sfp_bear") {
    return { family: "sfp", bear: signalKind === "sfp_bear" };
  }
  if (signalKind === "pullback" || signalKind === "pullback_bear") {
    return { family: "pullback", bear: signalKind === "pullback_bear" };
  }
  return { family: signalKind, bear: false };
}

/**
 * SFP / pullback: 30m pre-entry + full position + post-exit window (default 6h).
 */
function buildFocusedSnapshotBars(bars, trade, cfg) {
  ...
  const postExitMs = snapshotPostExitMs(chartCfg);
  const historyStartMs = trade.openedAt - preMs;
  const endMs = trade.closedAt + postExitMs;
  ...
  if (filtered.length < 10) {
    return {
      displayBars: filtered,
      meta: {
        chartStyle: "focused",
        preEntryMinutes: chartCfg.preEntryMinutes,
        postExitHours: postExitMs / (60 * 60 * 1000),
      },
    };
  }
  ...
  const { family, bear } = snapshotChartFamily(kind);
  const meta = {
    chartStyle: "focused",
    chartTheme: bear ? `${family}_bear` : family,
    bear,
    preEntryMinutes: chartCfg.preEntryMinutes,
    postExitHours: postExitMs / (60 * 60 * 1000),
  ...
  if (family === "sfp") {
    if (bear) {
      const sfp = entryWindow.length ? sweepRejectMetrics(entryWindow, chartCfg) : null;
      meta.sweepHigh = sfp?.sweepHigh ?? trade.sweepHigh ?? null;
      meta.sweepThreshold = sfp?.sweepThreshold ?? null;
      meta.rejectLevel = sfp?.rejectLevel ?? trade.corridorHigh ?? null;
    } else {
...
function buildLogSnapshotBars(bars, trade, cfg) {
  ...
  const tailEndIdx = barIndexAtOrBefore(filtered, endMs);
  const tailBars = filtered.slice(safeExit + 1, tailEndIdx + 1);
...
async function saveTradeSnapshot({
  ...
  try {
    const { notifyTradeSnapshotSaved } = require("./db/registry-hooks");
    notifyTradeSnapshotSaved({
      id: snapshotId,
      tradeId: trade.id,
      botType: snapshotKind === "backtest" ? "backtest" : "live",
      filePath,
      bytes: buffer.length,
    });
  } catch {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    try {
      const { snapshotId } = await saveTradeSnapshot({
        ...
        skipCleanup: true,
        snapshotPostExitHours: SNAPSHOT_POST_EXIT_HOURS,
        ...chartCfg,
      });
...
module.exports = {
  ...
  clearBacktestSnapshots,
  cleanOldSnapshots,
  SNAPSHOT_PRE_ENTRY_MINUTES,
  SNAPSHOT_POST_EXIT_HOURS,
  SNAPSHOT_MAX_AGE_MS,
  snapshotPostExitMs,
  snapshotExists,
  readTradeSnapshot,
