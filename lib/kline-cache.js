...
/** Read barCount / lastCloseTime from cache header without parsing the bars array. */
...
function readKlineCacheMeta(dir, symbol, interval) {
  ...
  for (const file of [primary, live]) {
    try {
      if (!fs.existsSync(file)) continue;
      const meta = readCacheFileMeta(file, interval);
      if (!meta) continue;
      if (!best || meta.barCount > best.barCount) best = meta;
    } catch {
      /* ignore */
    }
  }
...
function readKlineCache(dir, symbol, interval) {
  ...
  try {
    if (fs.existsSync(live) && live !== primary) {
