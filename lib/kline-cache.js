...
function createKlineCacheStore(options) {
  ...
  function persist(symbol, bars) {
    ...
    const existing = readKlineCache(dir, symbol, interval) ?? [];
    const merged = mergeBarsByOpenTime(existing, bars);
    writeKlineCache(dir, symbol, interval, merged, { maxBars, evalLimit });
  }
