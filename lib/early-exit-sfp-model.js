...
const MODEL_BASENAME = "early-exit-sfp";
...
function loadModelFromDisk(scope = "paper") {
  const raw = readJsonFile(MODEL_FILE(scope), null);
...
function evaluateAiEarlyExit(cfg, pos, bar, options = {}) {
  ...
  const model = normalizeStoredModel(options.model ?? getModel(modelScope));
  const hardThreshold =
...
function ensureDefaultModelOnDisk(scope = "paper") {
  const key = modelScopeKey(scope);
  const raw = readJsonFile(MODEL_FILE(key), null);
  if (!raw) {
    saveModel(
      {
        ...DEFAULT_MODEL,
