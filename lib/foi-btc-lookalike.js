...
const FOI_BTC_LOOKALIKE_DEFAULTS = {
  ...
  /**
   * When true, missing bars / insufficient samples block the open.
   * Live champion enables this; offline research may keep false.
   */
  foiBtcLookalikeFailClosed: false,
};
...
function normalizeFoiBtcLookalikeConfig(raw = {}) {
  ...
  return {
    ...
    foiBtcLookalikeFailClosed: Boolean(
      raw.foiBtcLookalikeFailClosed ?? d.foiBtcLookalikeFailClosed
    ),
  };
}
...
function foiBtcLookalikeAllows(cfgInput, options = {}) {
  ...
  if (!alt?.length || !btc?.length) {
    if (cfg.foiBtcLookalikeFailClosed) {
      return {
        ok: false,
        reason: "foi_btc_lookalike",
        detail: "missing_bars_block",
      };
    }
