...
async function trainFromTrades(trades, fetchBars, options = {}) {
  ...
  const kinds =
    options.signalKinds ??
    (options.foiOnly ? ["foi", "foi_bear"] : ["sfp", "sfp_bear"]);
  const trainTrades = (trades ?? []).filter((t) => kinds.includes(t.signalKind));
  const kindLabel = kinds.join("/");
  if (trainTrades.length < 12) {
    throw new Error(`Need at least 12 ${kindLabel} trades (got ${trainTrades.length})`);
  }

  onProgress?.({
    phase: "samples",
    message: `Building exit-level samples from ${trainTrades.length} ${kindLabel} trades…`,
  });

  const grouped = await buildTrainingSamples(trainTrades, fetchBars, cfg, { onProgress });
