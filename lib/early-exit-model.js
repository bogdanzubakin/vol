...
function shouldLabelEarlyExit(trade, adjustedPnl, finalPnl, margin, features) {
  const exitReason = String(trade.exitReason ?? "");
  if (exitReason === "take_profit") return false;
  if (finalPnl > 0.08) return false;

  const minBenefit = Math.max(0.15, margin * 0.045);
  const fav = (features.favorableMove ?? 0) * 5;
  const adv = (features.adverseMove ?? 0) * 5;
  const giveback = (features.givebackPct ?? 0) * 4;

  if (adjustedPnl > finalPnl + minBenefit) {
    if (exitReason === "stop_loss" && Math.abs(finalPnl) < minBenefit) {
      return adv >= 0.6 || giveback >= 1.4;
    }
    if (exitReason === "early_stall" && finalPnl >= -minBenefit) {
      return giveback >= 1.5 && adv >= 0.45;
    }
    return true;
  }
  ...
  if (
    ...
    giveback >= 1.2 &&
    fav >= 0.45 &&
    adjustedPnl > finalPnl + minBenefit * 0.6
  ) {
    return true;
  }
...
async function trainFromTrades(trades, fetchBars, options = {}) {
  ...
  let samples = await buildTrainingSamples(trades, fetchBars, cfg, { onProgress });
