#!/usr/bin/env node
/**
 * BP 1.0 + adverse_only early-abort (no stall / no invalidation) · 10d retest.
 *
 *   node scripts/eval-foi-1m-adverse-only-10d.js --days 10
 */
const fs = require("fs");
...
const OUT = () => dataPath("foi-1m-adverse-only-10d.json");
const TRADES_OUT = () => dataPath("foi-1m-adverse-only-10d-trades.json");
const BP_DIR = () => path.join(dataPath(), "breakpoints", "1.0");
const BP_BASELINE = () => path.join(BP_DIR(), "foi-1m-loss-reduce-10d.json");
const BP_TRADES = () => path.join(BP_DIR(), "foi-1m-loss-reduce-10d-trades.json");

function buildAdverseOnlyPatch() {
...
function applyLocalPatch(patch) {
  for (const name of ["paper-bot-state.json", "live-bot-state.json"]) {
    ...
    log(`Applied adverse_only patch → ${name}`);
  }
}

async function main() {
  const { days } = parseArgs(process.argv);
  const patch = buildAdverseOnlyPatch();
  ...
  log(`FOI 1m adverse_only eval · ${days}d · ${syms.length} symbols`);
  log(
    `EA: adverse ON · stall=${patch.earlyAbortStallEnabled} · invalidation=${patch.earlyAbortInvalidationEnabled} · bars=${patch.earlyAbortBars} · maxAdv=${patch.earlyAbortMaxAdversePct}`
  );
  ...
  for (let i = 0; i < syms.length; i += BATCH) {
    ...
    const { result } = await runPaperBotBacktest({
      ...
      modelScope: "paper",
      runMeta: { eval: "foi-1m-adverse-only-10d" },
    });
  ...
  const bpReport = readJsonFile(BP_BASELINE(), null);
  const bpSummary = bpReport?.summary ?? null;
  const bpTrades = readJsonFile(BP_TRADES(), null)?.trades;
  ...
  const report = {
    ...
    deltaVsBp10: bpSummary
      ? {
          pnl: +(summary.pnl - bpSummary.pnl).toFixed(2),
          trades: summary.trades - bpSummary.trades,
          winRate: +(summary.winRate - bpSummary.winRate).toFixed(1),
          lossPnl: +(summary.lossPnl - bpSummary.lossPnl).toFixed(2),
          winPnl: +(summary.winPnl - bpSummary.winPnl).toFixed(2),
          earlyExits: summary.earlyExits - (bpSummary.earlyExits ?? 0),
        }
      : null,
  };

  if (bpTrades?.length && report.deltaVsBp10) {
    const baseQuick = bpTrades.filter(
      (t) => (t.holdMin ?? 99) <= 15 && t.exitReason === "stop_loss"
    );
    report.deltaVsBp10.quickSlPnl = +(
      summary.quickSlLe15m.pnl - baseQuick.reduce((s, t) => s + t.pnl, 0)
    ).toFixed(2);
    report.deltaVsBp10.quickSlTrades = summary.quickSlLe15m.trades - baseQuick.length;
  }
  ...
  if (report.deltaVsBp10) {
    log(
      `Δ vs BP1.0: PnL ${report.deltaVsBp10.pnl >= 0 ? "+" : ""}$${report.deltaVsBp10.pnl} · trades ${report.deltaVsBp10.trades} · early ${report.deltaVsBp10.earlyExits}`
    );
  }
  log(`Saved ${OUT()}`);
  console.log(
    JSON.stringify(
      {
        summary,
        delta: report.deltaVsBp10,
        ea: {
          earlyAbortAdverseEnabled: patch.earlyAbortAdverseEnabled,
          earlyAbortStallEnabled: patch.earlyAbortStallEnabled,
          earlyAbortInvalidationEnabled: patch.earlyAbortInvalidationEnabled,
        },
      },
      null,
      2
