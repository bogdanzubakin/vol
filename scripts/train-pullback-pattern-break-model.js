#!/usr/bin/env node
/**
 * Train pullback pattern-break filter from pullback closed trades.
 *
 *   node scripts/train-pullback-pattern-break-model.js
 *   node scripts/train-pullback-pattern-break-model.js --source backtest
 *   node scripts/train-pullback-pattern-break-model.js --scope live
 */
...
async function main() {
  ...
  if (!trades.length) {
    console.error(
      scope === "live"
        ? "No pullback closed trades found. Run train bot with pullback signals enabled (live fills merged when available)."
        : "No pullback closed trades found — run train bot with pullback signals enabled."
    );
    process.exit(1);
  }
  ...
  console.error(
    `Training pullback pattern-break from ${trades.length} trades (scope=${scope} source=${source})…`
  );
