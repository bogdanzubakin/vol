#!/usr/bin/env node
/**
 * Single lever on adverse_only: SFP confirm only if |oiΔ1h| < 2 (PB unrestricted).
 * Keep SFP+PB confirms and adverse-only early abort.
 *
 *   node scripts/eval-foi-1m-sfp-oi-gate-10d.js --days 10
 */
const fs = require("fs");
...
const OUT = () => dataPath("foi-1m-sfp-oi-gate-10d.json");
const TRADES_OUT = () => dataPath("foi-1m-sfp-oi-gate-10d-trades.json");
...
function buildPatch() {
  ...
  return {
    ...base,
    // keep global OI=5; gate SFP confirm only when |oi|<2
...
function applyLocalPatch(patch) {
  for (const name of ["paper-bot-state.json", "live-bot-state.json"]) {
    ...
    log(`Applied sfp_oi_gate patch → ${name}`);
  }
}
...
async function main() {
  ...
  log(`FOI 1m sfp_oi_gate eval · ${days}d · ${syms.length} symbols`);
  log(
  ...
  for (let i = 0; i < syms.length; i += BATCH) {
    ...
    const { result } = await runPaperBotBacktest({
      ...
      modelScope: "paper",
      runMeta: { eval: "foi-1m-sfp-oi-gate-10d" },
    });
  ...
  else log(`SKIP apply: sfp_oi_gate $${summary.pnl} did not beat adverse $${adverseSummary?.pnl}`);
