#!/usr/bin/env node
/**
 * Single lever on BP 1.1 (adverse + SFP OI-gate): early_adverse OFF (SL/TP-only).
 * Keep SFP OI-gate |oi|<2 and global OI=5.
 *
 *   node scripts/eval-foi-1m-ea-off-10d.js --days 10
 */
const fs = require("fs");
...
const OUT = () => dataPath("foi-1m-ea-off-10d.json");
const TRADES_OUT = () => dataPath("foi-1m-ea-off-10d-trades.json");
const BP11_BASE = () => dataPath("foi-1m-sfp-oi-gate-10d.json");
const BP11_DIR = () => path.join(dataPath(), "breakpoints", "1.1");

function buildPatch() {
  const bp11 = readJsonFile(BP11_BASE(), null) ?? readJsonFile(path.join(BP11_DIR(), "foi-1m-sfp-oi-gate-10d.json"), null);
  const base = bp11?.patch ?? readJsonFile(path.join(BP11_DIR(), "patch.json"), {}) ?? {};
  return {
    ...base,
    foiConfirmSfpMaxOiDelta1hAbs: 2,
    // EA off alone — all early-abort paths disabled
...
async function main() {
  ...
  log(`FOI 1m ea_off eval · ${days}d · ${syms.length} symbols`);
  log(
    `OI global=${patch.foiMaxOiDelta1hAbs} · SFP OI-gate=${patch.foiConfirmSfpMaxOiDelta1hAbs} · SFP_bear=${patch.foiConfirmSfpBear} · PB_bear=${patch.foiConfirmPullbackBear}`
  );
  log(
  ...
  for (let i = 0; i < syms.length; i += BATCH) {
    ...
    const { result } = await runPaperBotBacktest({
      ...
      modelScope: "paper",
      runMeta: { eval: "foi-1m-ea-off-10d" },
    });
  ...
  const bp11Report = readJsonFile(BP11_BASE(), null);
  const bp11Summary = bp11Report?.summary ?? null;
  const deltaVsBp11 = bp11Summary
    ? {
        pnl: +(summary.pnl - bp11Summary.pnl).toFixed(2),
        trades: summary.trades - bp11Summary.trades,
        winRate: +(summary.winRate - bp11Summary.winRate).toFixed(1),
        earlyExits: summary.earlyExits - (bp11Summary.earlyExits ?? 0),
        winPnl: +(summary.winPnl - bp11Summary.winPnl).toFixed(2),
        lossPnl: +(summary.lossPnl - bp11Summary.lossPnl).toFixed(2),
      }
    : null;

  const improves = deltaVsBp11 ? deltaVsBp11.pnl > 0.01 : false;
  if (improves) applyLocalPatch(patch);
  else log(`SKIP apply: ea_off $${summary.pnl} did not beat BP1.1 $${bp11Summary?.pnl}`);
  ...
  if (deltaVsBp11) {
    log(
      `Δ vs BP1.1: PnL ${deltaVsBp11.pnl >= 0 ? "+" : ""}$${deltaVsBp11.pnl} · trades ${deltaVsBp11.trades} · applied=${improves}`
    );
  }
  log(`Saved ${OUT()}`);
  console.log(JSON.stringify({ summary, deltaVsBp11, applied: improves }, null, 2));
}
