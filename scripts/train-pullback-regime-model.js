#!/usr/bin/env node
/**
 * Train pullback regime filter model from pullback closed trades.
 *
 *   node scripts/train-pullback-regime-model.js
 *   node scripts/train-pullback-regime-model.js --source backtest
 *   node scripts/train-pullback-regime-model.js --scope live
 */
...
async function main() {
  ...
  console.error(
    `Training pullback regime from ${trades.length} trades (scope=${scope} source=${source})…`
  );
