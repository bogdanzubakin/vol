#!/usr/bin/env node
/**
 * FOI-only on 1m: both long+short enabled.
 * 1) Install involved 1m AI models (SFP / pullback / early-exit)
 * 2) Train AI exit-levels on 30d FOI long+bear trades
 * 3) Coordinate-descent over FOI + exit settings on 10d (parallel workers)
 *
 *   node scripts/optimize-foi-1m-both-10d.js --reset
 *   node scripts/optimize-foi-1m-both-10d.js --concurrency 4 --skip-funding-prefetch
 *   node scripts/optimize-foi-1m-both-10d.js --worker-train
 *   node scripts/optimize-foi-1m-both-10d.js --worker-eval
 */
...
const { reloadModel: reloadSfp, MODEL_FILE: SFP_MODEL_FILE } = require("../lib/sfp-regime-model");
const { reloadModel: reloadPbSignal, MODEL_FILE: PB_MODEL_FILE } = require("../lib/pullback-signal-model");
...
const INTERVAL = "1m";
const BATCH = 50;
const OUT_FILE = () => dataPath("foi-1m-both-10d-optimize.json");
const BEST_FILE = () => dataPath("foi-1m-both-10d-best.json");
const MODEL_DIR = () => path.join(dataPath(), "foi-1m-both-models");
const MODEL_FILE = () => path.join(MODEL_DIR(), "ai-exit-levels.json");
const INVOLVED_MODEL_DIR = () => path.join(dataPath(), "interval-10d-models", "1m");
...
function parseArgs(argv) {
  ...
  return {
    ...
    evalDays: Math.max(1, Math.min(30, Math.round(evalDays) || 10)),
    reset,
    maxSymbols: Math.max(0, Math.round(maxSymbols) || 0),
    minTrades: Math.max(1, Math.round(minTrades) || 40),
...
function createFetchers() {
  return {
    ...
    // Signal TF is already 1m — no separate exit bars needed
...
/** Install SFP / pullback / early-exit models trained for 1m (FOI confirm filters). */
function installInvolvedModels(scope = "paper") {
  const root = INVOLVED_MODEL_DIR();
  if (!fs.existsSync(root)) {
    log(`Involved models missing at ${root} — continue with current ${scope} models`);
    return false;
  }
  const installed = [];

  const sfpOnnx = path.join(root, "sfp-regime-onnx.json");
  const sfpLog = path.join(root, "sfp-regime-funding.json");
  if (fs.existsSync(sfpOnnx)) {
    installModelFile(sfpOnnx, SFP_MODEL_FILE(scope));
    const onnxSrc = path.join(root, "sfp-regime-onnx");
    if (fs.existsSync(onnxSrc)) {
      fs.mkdirSync(sfpOnnxDir(scope), { recursive: true });
      for (const name of fs.readdirSync(onnxSrc)) {
        const p = path.join(onnxSrc, name);
        if (fs.statSync(p).isFile()) fs.copyFileSync(p, path.join(sfpOnnxDir(scope), name));
      }
    }
    installed.push("sfp-regime");
  } else if (fs.existsSync(sfpLog)) {
    installModelFile(sfpLog, SFP_MODEL_FILE(scope));
    installed.push("sfp-regime-funding");
  }

  const pbOnnx = path.join(root, "pullback-signal-onnx.json");
  const pbLog = path.join(root, "pullback-signal-funding.json");
  if (fs.existsSync(pbOnnx)) {
    installModelFile(pbOnnx, PB_MODEL_FILE(scope));
    const onnxSrc = path.join(root, "pullback-signal-onnx");
    if (fs.existsSync(onnxSrc)) {
      fs.mkdirSync(pbOnnxDir(scope), { recursive: true });
      for (const name of fs.readdirSync(onnxSrc)) {
        const p = path.join(onnxSrc, name);
        if (fs.statSync(p).isFile()) fs.copyFileSync(p, path.join(pbOnnxDir(scope), name));
      }
    }
    installed.push("pullback-signal");
  } else if (fs.existsSync(pbLog)) {
    installModelFile(pbLog, PB_MODEL_FILE(scope));
    installed.push("pullback-signal-funding");
  }

  const early = path.join(root, "early-exit-sfp.json");
  if (fs.existsSync(early)) {
    installModelFile(
      early,
      dataPath(scope === "live" ? "early-exit-sfp-live.json" : "early-exit-sfp.json")
    );
    installed.push("early-exit");
  }
  ...
  clearPbOnnx();
  reloadEarlyExit(scope);
  log(`Involved 1m models → ${scope}: ${installed.join(", ") || "(none)"}`);
    ...
    const results = await Promise.all(
      wave.map((batch) =>
        runPaperBotBacktest({
          ...
          modelScope: "paper",
          runMeta: { optimize: "foi-1m-both-10d", label },
        })
      )
    );
...
async function trainWorker(job) {
  ...
  const collect = await runFoiBacktest({
    label: `1m_train_collect_${trainDays}d`,
  ...
  const model = await trainExitLevels(collect.closedTrades, fetchBarsTradeWindow, {
    botConfig: baseBot,
    scope: "paper",
    source: `foi-1m-both:${trainDays}d`,
...
async function main() {
  ...
  log(
    `FOI 1m BOTH · train ${trainDays}d · eval ${evalDays}d · ${symbols.length} symbols · concurrency=${concurrency}`
  );
