#!/usr/bin/env node
/**
 * Audit winning FOI 15m both-sides config on 5d:
 ...
 *
 *   node scripts/audit-foi-15m-winner-5d.js --days 5
 *   node scripts/audit-foi-15m-winner-5d.js --days 30
 */
const fs = require("fs");
...
const INTERVAL = "15m";
const BATCH = 50;
const OUT = (days) => dataPath(`foi-15m-winner-${days}d-audit.json`);
const TRADES_OUT = (days) => dataPath(`foi-15m-winner-${days}d-trades.json`);
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    if (argv[i] === "--days" && argv[i + 1]) days = Math.max(1, Number(argv[++i]) || 5);
  }
  return { days };
}
...
function createFetchers() {
  ...
  function signalBars(sym, barCount) {
    ...
    if (!agg.has(symbol)) agg.set(symbol, aggregateBars(raw, 15));
    return sliceTail(agg.get(symbol), barCount);
  }
  return {
    signalBars,
    async fetchKlinesForSymbol(sym, n) {
      ...
      throw new Error(`no 15m ${sym}`);
    },
    async fetchKlines1mForSymbol(sym, n) {
      const c = sliceTail(bars1m(sym), n);
      if (c?.length >= 200) return c;
...
function loadWinnerBot() {
  const best = readJsonFile(dataPath("foi-15m-both-5d-best.json"), null);
  ...
  const local = readJsonFile(dataPath("paper-bot-state.json"), null)?.config ?? {};
...
function installWinnerModel() {
  const src = path.join(dataPath(), "foi-15m-both-models", "ai-exit-levels.json");
  if (!fs.existsSync(src)) {
    log(`WARN: no FOI 15m model at ${src} — using current paper model`);
    return;
  }
...
async function main() {
  ...
  log(`Audit FOI 15m winner · ${days}d · ${syms.length} symbols`);
  log(
  ...
  for (let i = 0; i < syms.length; i += BATCH) {
    ...
    const { result } = await runPaperBotBacktest({
      ...
      modelScope: "paper",
      runMeta: { audit: `foi-15m-winner-${days}d` },
    });
