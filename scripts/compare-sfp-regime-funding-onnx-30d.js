#!/usr/bin/env node
/**
 * Compare SFP regime filter variants over 30d:
 ...
 *
 *   node scripts/compare-sfp-regime-funding-onnx-30d.js --days 30
 *   node scripts/compare-sfp-regime-funding-onnx-30d.js --days 30 --skip-prefetch
 */
...
const OUT_FILE = () => dataPath("sfp-regime-funding-onnx-compare.json");
const PROGRESS_FILE = () => dataPath("sfp-regime-funding-onnx-progress.json");
const BASELINE_MODEL = () => dataPath("sfp-regime-model-baseline.json");
const FUNDING_MODEL = () => dataPath("sfp-regime-model-funding.json");
const SAMPLES_FILE = () => dataPath("sfp-regime-onnx-samples.json");
...
function summarize(result) {
  ...
  const sfp = closed.filter((t) => t.signalKind === "sfp" || t.signalKind === "sfp_bear");
  let sfpPnl = 0;
  let tp = 0;
  for (const t of sfp) {
    sfpPnl += Number(t.pnl) || 0;
  ...
  return {
    ...
    tpRate: sfp.length ? +((100 * tp) / sfp.length).toFixed(1) : 0,
    winRate: s.closedCount
      ? +((100 * (s.winCount ?? 0)) / s.closedCount).toFixed(1)
      : 0,
    sfpRegimeSkips: s.sfpRegimeSkips ?? 0,
    sfpRegimeSkipsBull: s.sfpRegimeSkipsBull ?? 0,
    sfpRegimeSkipsBear: s.sfpRegimeSkipsBear ?? 0,
...
async function runBacktest({
  ...
  const { result } = await runPaperBotBacktest({
    ...
    getFundingOiAt,
    runMeta: { compare: "sfp-regime-funding-onnx", label },
    onProgress: (p) => {
  ...
  log(
    `→ $${row.pnl} · ${row.trades} tr · SFP ${row.sfpTrades} ($${row.sfpPnl}) · TP ${row.tpRate}% · regime skip ${row.sfpRegimeSkips}`
  );
  return row;
}

function sfpTrainBase(saved) {
...
async function collectSfpTrades({ days, symbols, signalCfg, fetchers, getFundingOiAt }) {
  const saved = readJsonFile(dataPath("paper-bot-state.json"), {})?.config ?? {};
  const botConfig = sfpTrainBase(saved);
  const { result } = await runPaperBotBacktest({
    symbols,
    signalCfg,
    botConfig,
    ...
    saveLastResult: true,
    getFundingOiAt,
    runMeta: { compare: "sfp-regime-funding-onnx", label: "sfp_train_collect" },
  });
  const trades = (result.closedTrades ?? []).filter(
    (t) => t.signalKind === "sfp" || t.signalKind === "sfp_bear"
  );
  log(`Collected ${trades.length} SFP trades for training`);
  return trades;
}
...
function installRegimeModel(file, onnx = false) {
...
async function trainVariant({ trades, featureNames, fundingOi, source, outFile, trainOpts = {} }) {
  ...
  const model = await trainFromTrades(trades, fetchBars, {
    ...
    aiSfpRegimeFundingOiEnabled: featureNames.length > FEATURE_NAMES.length,
...
function trainOnnxModels(samplesPath, outDir) {
  ...
  const res = spawnSync(
    ...
    [
      ...
      outDir,
      "--feature-count",
      String(FEATURE_NAMES_WITH_FUNDING_OI.length),
      "--prefix",
      "sfp-regime",
      "--bull-kind",
      "sfp",
...
async function sweepBest({
  ...
  installRegimeModel(modelFile, onnx);
  let best = null;
  const runs = [];
  for (const th of REGIME_THRESHOLDS) {
    const botConfig = {
      ...botBase,
      aiSfpRegimeEnabled: true,
      aiSfpRegimeBullThreshold: th.bull,
      aiSfpRegimeBearThreshold: th.bear,
      aiSfpRegimeFundingOiEnabled: onnx || variant.includes("funding"),
      aiSfpRegimeOnnxEnabled: onnx,
      ...extraPatch,
    };
...
async function main() {
  ...
  log(`SFP regime funding/OI vs ONNX · ${days}d · ${symbols.length} symbols`);
  ...
  const trades = await collectSfpTrades({
    days,
    symbols,
    signalCfg,
    fetchers,
  ...
  if (trades.length < 30) {
    throw new Error(`Need >=30 SFP trades (got ${trades.length})`);
  }
  ...
  const baselineModel = await trainVariant({
    ...
    fundingOi: null,
    source: "compare:regime-baseline",
  ...
  const fundingModel = await trainVariant({
    ...
    source: "compare:regime-funding-oi",
  ...
  const baselineSweep = await sweepBest({
    ...
    extraPatch: { aiSfpRegimeFundingOiEnabled: false },
  });
  ...
  const fundingSweep = await sweepBest({
    ...
    extraPatch: { aiSfpRegimeFundingOiEnabled: true },
  });
  saveCheckpoint({ phase: "funding_sweep_done", fundingSweep: fundingSweep.best });

  let onnxSweep = null;
  if (onnxTrain.ok && fs.existsSync(path.join(onnxOut, "sfp-regime-bull.gbm.json"))) {
    const onnxModel = {
      ...fundingModel,
      backend: "onnx",
      featureNames: FEATURE_NAMES_WITH_FUNDING_OI,
      source: "compare:regime-onnx-gbm",
      trainedAt: Date.now(),
    };
    writeJsonFile(dataPath("sfp-regime-model-onnx.json"), onnxModel);
    onnxSweep = await sweepBest({
      ...
      fetchers,
      getFundingOiAt,
      modelFile: dataPath("sfp-regime-model-onnx.json"),
      onnx: true,
      extraPatch: {
        aiSfpRegimeFundingOiEnabled: true,
  ...
  const out = {
    ...
    ranking,
    integration: {
      features: "lib/sfp-regime-features.js → extractSfpRegimeFeatures(extras.fundingOi)",
      cache: "lib/funding-oi-cache.js → getFundingOiAt(symbol, asOfMs)",
      onnx: "lib/sfp-regime-onnx.js → GBM JSON (backtest) + .onnx (live)",
    },
  };
